...
> **R55 CURRENT BOUNDARY:** The historical P0.2 flat-arm architecture and the defective R54/P0.1 candidate remain withdrawn. `HR-V0-ARM-ARCH-P0.2` provides corrected XM540/S102 registration, vertical 20-2040 link candidates, explicit transforms and a 221-pose collision screen, but `HR-V0-MECH-P0.3` remains the release hold. No adapter, beam, fastener, tolerance, motion limit or supplier packet is released.

**PRELIMINARY—NOT APPROVED FOR ENERGIZATION**

Status: correction-stage feasibility model. Exact vendor-coordinate arm source, explicit replacement transforms, an interactive candidate model, preliminary mass properties, reproducible structural screens, three controlled fit-coupon packages, a proposed orderable gripper mechanism allocation, a checked hard-stop kinematic/load-case study, and a generated guard/catch/cable space study now exist. Adapter local-strength proof, exact fastener stacks, physical coupon/kit inspection, released production tolerances/drawings, complete mass closure, fabricable hard-stop parts, measured stopping/drop/sweep evidence, exact guard/receiver/harness parts, gripper force/fasteners, bench anchors, proof tests, and mechanical release are not complete. See [the corrected R55 arm architecture candidate](hr-v0-arm-architecture-p0.2.md), [the R0.1 historical baseline](hr-v0-mechanical-r0.1.md), [joint-interface basis](hr-v0-joint-interface-fasteners-p0.1.md), [gripper architecture](hr-v0-gripper-architecture-p0.1.md), [guard/receiver/cable architecture](hr-v0-guard-receiver-cable-p0.1.md), [hard-stop basis](hr-v0-hard-stop-design-basis-p0.1.md), and [R11 engineering calculations](r11-engineering-calculations.md).

## Current exact-coordinate candidate

R55 registers the raw XM540 body to the S102 body frame with the controlled vendor-to-joint rotation, while keeping the reference J1 and J2 axes parallel. Candidate J1-to-J2 spacing is 193.025 mm. The H104 gripper-frame origin is 119.525 mm beyond J2 and 312.55 mm from J1, reserving 47.45 mm to the 360 mm object-center ceiling. The upper/forearm members are 100/50 mm vertical `20-2040` conservative collision envelopes between candidate 4.7625 mm adapters. A 221-pose, 0.5° screen from 15° through 125° finds first nominal adapter/body contact at 122°; 120° is therefore only a provisional software ceiling. Cables, hard-stop/stopping overtravel, continuous-between-sample proof, guards, base/column, gripper mechanism and tolerances remain excluded.

The updated allocation screen is 1.777 N·m shoulder gravity and 0.483 N·m elbow gravity; after the existing 2.25 screening multiplier, 3.999 N·m and 1.086 N·m. The candidate 20-2040 member bending and M5 end-thread screens pass using published 80/20 section/yield data, but the adapter countersink leaves only 1.6625 mm nominal residual material and local pull-through/bending is unresolved. These values are non-physical screens, not allowable loads, continuous actuator ratings or structural proof.
...
## Historical P0.2 static gravity torque — superseded by R55 candidate screen
