# HR-V0 protection and conductor coordination P0.1

> **PRELIMINARY - NOT APPROVED FOR FABRICATION OR ENERGIZATION**

Date: 2026-08-06

Applies to: Electrical V3-P1.1, references `F0`, `F1`, `F2`, `F3`, `FSR1`, and `FSR2`

Status: input and evidence-control package only; physical values and execution evidence remain open

## Decision

No fuse ampere rating is released. R36 freezes a candidate fuse family and two candidate holder/distribution products so the physical coordination work has a concrete boundary, but it does not select a fuse rating, conductor, terminal, splice, cable length, or enclosure layout.
...
- `FSR1` and `FSR2`: exact device family, rating, holder, conductors, and terminals remain `SELECTION REQUIRED`. The Schneider `LC1D25BD` coil's published 5.4 W value gives a nominal arithmetic screen of 0.225 A at 24 V; it does not establish pickup, transient, fault, or protective-device behavior.
...
This pass advances energization gate `EG-014` from `open` to `partial` because the six-reference input register, candidate fuse/holder boundary, exact evidence fields, execution template, and machine check now exist. It does not close the gate. All six ratings, the two XM540 connector-limit conflicts, physical harness construction, fault current, operating current, thermal data, clearing proof, installation review, and qualified approval remain unresolved.
