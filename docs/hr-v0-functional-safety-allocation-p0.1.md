...
R86 proved that placing ordinary KWD contacts in the SR1 input returns created an unacceptable candidate injection path. R87 Electrical V3-P1.13 removes every KWD terminal from those returns: both fixed E-stop NC channels now connect directly from SR1 through S0 and back to SR1. KWD1/KWD2 instead series-gate only `SR1:A1`. This removes the source-encoded internal KWD-to-E-stop-return path, but it does not prove physical noninterference. `WDF-008`, `WDF-011`, and `WDF-012..016` remain controlled open for external bridges, contamination, internal/pole-to-pole faults, gate defeat, routing, and qualified analysis. See `docs/hr-v0-watchdog-supply-gate-p0.1.md`.
...
- `EG-012` remains partial. R87 supplies a synchronized P1.13/P0.5 correction, 14 current paths, 32 open failure modes, 28 unexecuted cases, 12 separation controls and ten explicit holds. The encoded KWD-to-E-stop-return injection path is removed, but physical noninterference, qualified allocation, protected routing, contact application, fault injection, restart behavior and timing evidence remain open.
