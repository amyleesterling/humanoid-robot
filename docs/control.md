...
The repository supervisor configuration deliberately retains unresolved configuration/kinematic hashes and start-pose tolerances, so it fails closed. Seventeen unit tests and a source-hash manifest are present. R30 retains the frozen Pico GPIO, exact heartbeat and two-package driver candidates and adds exact proposed order codes for all ISO1212DBQ feedback passives, but no compiled RP2040 platform binding, released PCB, received/derating evidence, DYNAMIXEL transport, HIL trace or qualified code/safety review exists. See `docs/hr-v0-firmware-p0.1.md`, `docs/hr-v0-watchdog-feedback-p0.1.md`, `docs/hr-v0-heartbeat-driver-closure-r29.md`, and `docs/hr-v0-watchdog-feedback-passive-closure-r30.md`. No safety credit is assigned to this implementation.
