...
The repository supervisor configuration deliberately retains unresolved configuration/kinematic hashes and start-pose tolerances, so it fails closed. Seventeen unit tests and a source-hash manifest are present. R29 retains the frozen candidate Pico GPIO numbers and calculated ISO1212DBQ feedback circuit, and adds an exact VO618A heartbeat front end plus two separate TPL7407LPWR driver packages, but no compiled RP2040 platform binding, released PCB, DYNAMIXEL transport, HIL trace or qualified code/safety review exists. See `docs/hr-v0-firmware-p0.1.md`, `docs/hr-v0-watchdog-feedback-p0.1.md`, and `docs/hr-v0-heartbeat-driver-closure-r29.md`. No safety credit is assigned to this implementation.
