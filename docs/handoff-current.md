...
Mechanical package: **HR-V0-MECH-P0.2 coordination candidate plus HR-V0-MECH-R0.1-PRELIMINARY native geometry and HR-V0-PLATE-RFQ-P0.1 process/FAI control**
...
BOM package: **HR-V0-BOM-P0.1 closure candidate; 17 evaluation-only lines; 15 exact candidates on hold; 31 selection-required groups; no complete procurement release**
...
- The custom-metal route uses four flat 6061-T6 part definitions: one upper link, one forearm link, one shoulder adapter, and two bench-anchor plates. R50 issues `HR-V0-FAB-RFQ-P0.1`: `MV0-001` through `MV0-003` may be quoted as one-stop CNC or as a hole-free profile blank plus qualified secondary CNC/drilling. Xometry and Protolabs are one-stop candidates; SendCutSend is a profile-blank candidate only; Artisans Asylum requires direct capability confirmation. `MV0-004` remains on Boston bench-survey hold. No supplier or cutting order is released.
- R51 issues `HR-V0-FAB-RFI-P0.1`: three deterministic capability/DFM ZIPs with route-specific file exclusions and exact hashes. The profile packet contains only zero-hole blanks; no packet contains `MV0-004`. `MECH-004` and `INSPECT-MECH-011` now require one exact surveyed bench, permission, exact anchor selection/calculation and qualified proof evidence.
- R52 corrects the R51 packet representation after a fresh-clone failure exposed CRLF/LF-dependent payload hashes. Controlled text members are now canonical LF, the checker independently enforces source equivalence, and a fresh clone must validate committed packets before producing a zero-diff regeneration.
...
- 73 draft requirements
- 93 controlled verification procedure records
- 40 open risks
...
- 71 system BOM groups: 17 evaluation candidates, 15 exact candidates on hold, three grouped-component holds, 31 selection-required groups, four historical/DNP exclusions, and one integrated item
...
Fifty-two review/control rounds are complete and recorded in `docs/review-ledger.md`. R11 Fable and R12 Sol were commissioned independently against GitHub `main` at `ee276af...` before the R13/R14 corrections. R11 reported 7 BLOCKER, 11 MAJOR, and 12 MINOR findings. R12 reported 18 BLOCKER, 30 MAJOR, and 8 MINOR findings; the resupplied analysis is the same R12 verdict and is not double-counted. R13-R52 are controlled project responses, not independent approvals. R37 freezes the DXL-star board pin allocation and native routing but does not close its cable, protection, thermal, waveform, no-backfeed or physical-evidence gates. R38 adds a fail-closed actuator readback/current test candidate while explicitly leaving the branch-current and connector application conflict open. R39 adds exact Pico GPIO binding, publisher-pinned tools and reproducible target artifacts while keeping target execution and HIL open. R40 corrects the C/Python clock mismatch and executes compiled C against 44 controlled model steps. R41 adds current Schneider K1/K2 application evidence while explicitly retaining the critical-current/application and physical-test blocker. R42 adds a fail-closed IDEC received-lot terminal-control route while retaining all four RESET/ARM terminal identifiers as unresolved. R43 controls the custom flat-plate RFQ/FAI route while leaving every part dimensionally unreleased pending physical evidence. R44 removes all safety credit from the ordinary heartbeat diagnostic, separates the fixed physical guard from safety-related control functions, and creates the qualified PLr/SIL allocation and watchdog non-interference evidence routes. R45 binds the complete package to deterministic release-candidate metadata and hashes. R46 exposes the missing BOM dependency groups and controls seventeen evaluation-only candidate lines. R47 creates the controlled mechanical datum/interface contract, exact extrusion cuts and general arrangement while keeping every physical release hold open. R48 attempted an exact frame-joint selection; R49 found its two-slot-wide bracket, member-overlap and upright-placement defects, superseded it with `HR-V0-FRAME-P0.2`, and retained every physical fit, access, torque, proof and qualified-disposition hold. R50 adds RFQ-ready one-stop and two-process fabrication routes plus native hole-free blanks. R51 makes supplier inquiry and exact-bench anchor evidence executable while leaving every physical release and qualified review open. R52 corrects packet portability and proves the corrected representation in a clean clone without closing any physical gate.

## Principal unresolved engineering blockers

1. HR-V0 now has `HR-V0-MECH-P0.2` plus corrected `HR-V0-FRAME-P0.2`: a 24-row parameter contract, 12-interface register, 20-group assembly schedule, five exact extrusion cuts totaling 1980 mm, six enumerated frame joints using six `40-4332` brackets and twelve `75-3422` assemblies, six controlled bracket ridges, a fail-closed no-positive-volume member-overlap check, a six-datum neutral chain, corrected GLB/STEP assembly-space transforms, readable general arrangement, native custom-part DXF/STEP quote geometry, vendor CAD provenance, three controlled interface coupons, hard-stop studies, a proposed exact RM-X52 gripper allocation, guard/catch space envelope, five cable zones, and a 13-row moving-mass ledger. The 13–20 N m manufacturer value is only a trial guide; received bracket/gusset/tool access, actual-joint torque, slip/proof and qualified disposition remain open. Structural STL is prohibited after distinct link patterns produced identical meshes. The mass ledger still leaves only 184.6 g for unresolved moving hardware and is not a pass. The guard dimensions remain a space reservation, not a safety distance. Received fit, remaining fasteners, sweep/stopping/drop evidence, guard/receiver/harness parts, force/current limit, fabricable stops, bench anchoring, measured mass/COM/inertia, FAI, proof tests and independent review remain open. HR-30 has no released mechanical CAD.
...
9. `HR-V0-FW-P0.1` now supplies a fail-closed source-level state-machine candidate and `HR-V0-ACT-P0.1` register-readback rules. `HR-V0-WD-BUILD-P0.2` supplies an exact Pico GPIO binding plus nine byte-identical target artifacts from two clean, publisher-pinned builds. `HR-V0-WD-HOST-VECTOR-P0.1` executes the portable C and matches the Python model for 44 steps in nine scenarios, bringing the combined executable-test count to 29. The target binary has not been flashed or executed on received hardware, and there is still no Raspberry Pi deployment, DYNAMIXEL transport, received actuator identity, external branch-current limit, selected kinematics, bus timing measurement, HIL evidence, or reset-to-motion fault trace. Raw 800/300 current values are guarded test candidates only. HR-30 still lacks its selected real-time implementation.
...
The R34 reconciliation is `docs/reviews/2026-08-06-sol-r12-post-r34-status.md`. R35 records PCB-P0.5's 6 mil fabrication envelope; R36 records the machine-controlled protection inputs and connector/rating conflict; R37 records the native DXL-star correction; R38 records the actuator configuration/current-envelope candidate and still-open external-current evidence; R39 records the Pico target and reproducible build; R40 records the clock-semantics fix and compiled-C/model differential evidence; R41 records current contactor evidence while retaining the Schneider application, protection, received and loaded-test blockers; R42 through R51 have their matching post-round files; and the current R52 reconciliation is `docs/reviews/2026-08-07-sol-r12-post-r52-status.md`. R42 keeps EG-011 partial. R43, R50 and R51 keep EG-006 partial. R44 keeps EG-012 partial. R45 keeps EG-002 partial. R46 advances EG-003 to partial. R47-R52 keep EG-005 through EG-008 partial while correcting the mechanical evidence chain and packet representation. All are project-owned reconciliations, not new independent reviews, and all preserve Sol's physical-build, fabrication-release, functional-safety and energization blockers.

## Requested independent-review output

Use `docs/reviews/2026-08-06-electrical-v3-independent-review-request.md` for the controlled V3-P1.4 / PCB-P0.5 / DXL-STAR-P0.1 electrical review scope and reproduction commands. Use `docs/reviews/2026-08-07-firmware-p0.2-independent-review-request.md` for the corrected watchdog, compiled-C evidence, reproducible Pico build and supervisor review scope.
...
Submit V3-P1.4, PCB-P0.5, DXL-STAR-P0.1, the R36 protection package, HR-V0-ACT-P0.1, HR-V0-FW-P0.1 and watchdog P0.2 evidence for detailed independent electrical, routed-layout, controls and functional-safety review. Do not generate fabrication outputs until the selected suppliers accept final archives and the physical test-access, protection, schematic-parity, CAM and independent-layout gates close. Acquire and receive-inspect the frame, gripper and actuator kits; make all three coupons; and execute the controlled unpowered mechanical inspections. Receive S0/S1/S2 and the JA1/DC1 test articles under their controlled procedures, including the lot-specific S1/S2 terminal map. Before selecting a fuse, close the protection analysis and guarded single-axis fixture plan, including the XM540/JST conflict and external branch-current measurement. Obtain Schneider's identifiable K1/K2 application disposition after measuring the actual break/regeneration envelope. After separate reviewed fabrication releases, inspect both boards, build only controlled harness test articles, then execute DXL continuity/isolation/no-backfeed/thermal/waveform tests and disconnected-load watchdog P0.2 HIL. Freeze the 24 unresolved terminals and every remaining protection/conductor/interface selection. Do not issue a build or energization release until every applicable gate record closes.
