...
Electrical package: **Project Button Electrical V2.1 reviewed baseline; V3-P1.1 / PCB-P0.5 correction candidates**
...
The authoritative repository controls engineering intent and contains the reviewed Electrical V2.1 KiCad source plus the separate V3-P1.1 schematic and PCB-P0.5 routed/test-access/fabrication-envelope correction candidates. The website is a synchronized presentation and must not override the source specification. Individual documents retain their own revisions; neither electrical identifier replaces the `HR-30-SYS-R0.2` systems-package baseline. V3 does not supersede V2.1 until its open selections, calculations, tests, and qualified reviews close.
...
The V3-P1.1 candidate separately contains thirteen native pages, 78 component blocks, 299 modeled terminals, 100 native nets (63 named connected nets plus 37 deliberate auto-generated unconnected nets), 262 unique wire labels, 66 unresolved component/interface rows, and 46 `TBD-*` terminal designations. It retains P0.4's exact ISO1212DBQ circuit, P0.5's RESET/ARM and compute-source identities, P0.6's controlled S0 contacts, P0.7's JA1/DC1 selections, P0.8's exact heartbeat/driver candidates, P0.9's exact feedback passives and P1.0's exact board terminals/pin allocation. P1.1 adds 16 proposed Harwin test-point records. PCB-P0.5 contains 42 schematic references, four board-only holes, 201 segments, 56 vias and three filled B.Cu zones. Native DRC reports zero violations and zero routed unconnected pads; the independent checker proves a 0.1524 mm minimum trace width, 0.30 mm minimum via drill, 0.15 mm minimum annular ring, all multi-pad nets connected, 14 unused singletons isolated, both SUB thermal nets confined to their own pad/route/2 mm x 2 mm plane copper, and 89 no-net pads untouched. R32 corrected the non-matching ISO1212 footprint and field/control placement; R33 added routed copper; R34 added exact test access and SUB copper; R35 removed 0.10 mm routing and records a proposed OSH Park U.S. two-layer process envelope. R36 adds a six-reference protection register with exact proposed holder/distribution hardware and fuse family but zero released ampere ratings; it explicitly blocks the XM540 branches on the 4.4 A stall versus 3 A JST EH series conflict. Source-side construction/current division, fuse ratings, conductors/terminals, received/thermal/fault proof, RESET/ARM terminals, panel human factors, supplier archive acceptance, physical test access, thermal/pulse/DC-bias/EMC, COM-slew, HIL/fault evidence and qualified review remain open. No fabrication outputs exist. KiCad 10.0.5 ERC remains 0 errors and 0 warnings, and native netlist/PDF/SVG exports plus the exact-net, protection and PCB checkers pass. These V3 counts must not be substituted for the independently reviewed V2.1 counts above.
...
Thirty-six review/control rounds are complete and recorded in `docs/review-ledger.md`. R11 Fable and R12 Sol were commissioned independently against GitHub `main` at `ee276af...` before the R13/R14 corrections. R11 reported 7 BLOCKER, 11 MAJOR, and 12 MINOR findings. R12 reported 18 BLOCKER, 30 MAJOR, and 8 MINOR findings; the analysis resupplied on 2026-08-06 is the same R12 verdict and is not double-counted. R13-R36 are controlled project responses, not independent approvals. R29 freezes the exact heartbeat/driver path, R30 freezes the feedback passive order codes, R31 freezes the PCB boundary/terminal allocation, R32 corrects the native PCB footprint and placement constraints, R33 adds the DRC-clean routed-copper candidate, R34 adds exact test access plus floating ISO1212 SUB copper, R35 records the 6 mil supplier-process envelope, and R36 makes protection inputs machine-controlled without releasing fuse values. Supplier acceptance, received evidence, fault/HIL, protection and qualified-review blockers remain open.
...
5. V3 records exact E-stop, RESET/ARM, safety-relay, watchdog-relay, ISO1212 feedback-receiver and support-passive, source, contactor, U2D2, actuator, frame, JA1 project-side and DC1 regulator candidates where primary evidence permits. R36 additionally proposes the F0 holder, F1-F3 distribution block and F0-F3 fuse family, but all ampere ratings, application approval, conductors, terminals/splices, remaining connectors, enclosure and physical proof remain selection-dependent. Forty-six V3 terminals remain deliberately `TBD-*`; the E-stop contact positions, feedback IC/passive identities, JA1 project-side pins/contacts and DC1 pins are exact at the ECAD level, while received proof/derating, source-side current sharing, RESET/ARM terminals and the physical board/interface remain unreleased.
...
The R34 reconciliation is `docs/reviews/2026-08-06-sol-r12-post-r34-status.md`. R35 is `docs/reviews/2026-08-06-sol-r12-post-r35-status.md`; it records PCB-P0.5's 6 mil fabrication envelope. The current R36 reconciliation is `docs/reviews/2026-08-06-sol-r12-post-r36-status.md`; it records the machine-controlled protection inputs and still-open connector/rating conflict. All are project-owned reconciliations, not new independent reviews, and all preserve Sol's physical-build, fabrication-release, functional-safety and energization blockers.

## Requested independent-review output

Use `docs/reviews/2026-08-06-electrical-v3-independent-review-request.md` for the controlled V3-P1.1 / PCB-P0.5 electrical review scope and reproduction commands. Use `docs/reviews/2026-08-06-firmware-p0.1-independent-review-request.md` for the source-level watchdog and supervisor review scope.
...
Submit V3-P1.1, PCB-P0.5, the R36 protection package and HR-V0-FW-P0.1 for detailed independent electrical, routed-layout, controls and functional-safety review. Do not generate fabrication outputs until the selected supplier accepts the final archive and the physical test-access, protection, schematic-parity, CAM and independent-layout gates close. Acquire and receive-inspect the H101/S102 frame kits and proposed RM-X52 gripper kit; make all three coupons; execute `INSPECT-MECH-003`/`004`/`008` plus `INSPECT-GRIP-001`; and weigh every received/moving item under `INSPECT-MECH-007`. Receive S0/S1/S2 and execute `INSPECT-ELEC-003`; receive and build only the controlled JA1/DC1 test articles under `INSPECT-ELEC-004`. Before selecting any fuse value, close `ANALYSIS-ELEC-001`, `INSPECT-ELEC-008` and the guarded fixture plan for `TEST-ELEC-006`, including the XM540/JST connector conflict. After a reviewed fabrication release, execute `INSPECT-ELEC-006` and `INSPECT-ELEC-007`, then execute `TEST-ELEC-005` with relay/contactor/actuator loads disconnected. Build only an unpowered controlled mechanical article to freeze stop planes, cable loops, guard sweep and tolerance inputs. Then release the backed-up bumper/catch, guard/receiver and brackets before any powered mechanical test. In parallel, freeze the 46 unresolved terminals and all remaining protection/conductor/interface selections; compile reproducible firmware binaries and prepare disconnected-load HIL. Do not issue a build or energization release until every applicable gate record closes.
