...
The system BOM now contains 86 configuration groups: 17 evaluation candidates, 40 exact candidates on hold, three grouped-component holds, 19 selection-required groups, four exclusions and three integrated items. R46 exposed previously invisible assembly assumptions; later controlled passes add exact source, enclosure, compute-carrier and retention candidates. R122 holds the Pi-to-U2D2 cable; R123 separates exact rail, duct and DR1-DR3 end-bracket candidates into `BOM-083` through `BOM-085` while leaving residual `BOM-059` unresolved; R147 advances `BOM-063` to one exact held Eaton `P006-006` actuator-source AC cord candidate; R148 reconciles `BOM-027` to the exact held P0.7 C01/C04/C05/C06/C07 custom set; R149 binds `BOM-048` to current PCB-P0.9 / Electrical V3-P1.14 and `HR-V0-WD-PCBA-DATA-P0.2`; R151 binds `BOM-051` to DXL-STAR-P0.1 and its quarantined manufacturing-review package; R152 integrates obsolete three-module `BOM-035`; R153 adds integrated `BOM-086` for the three factory-included branch cables and limits loose connector/contact allocation to the custom controller cable. None is a complete procurement, fabrication or application release.
...
R62 advances `BOM-019` to an exact-candidate hold for two Phoenix `PT 4-HESI (5X20)` item `3211861` holders and adds `BOM-072` for the two still-unresolved fuse links. R63 adds exact-candidate-hold `BOM-073` for one Phoenix `D-ST 4` item `3030420` end cover. R64 advances `BOM-042` to exact-candidate hold for Littelfuse `75920-01`; conductor/lug, source-fault, load-break, touch-protection, cutout, zero-energy/padlock, human-factors, Boston application and physical tests remain open. Those historical counts have since been superseded by the current 86-group closure totals above. Both FSR fuse links, received accessory compatibility/grouping, all protection calculations and all physical evidence remain open; manufacturer maximum ratings are not project fuse or conductor ratings.
...
R149 supersedes the stale live PCB-P0.5 description for current configuration use. `BOM-048` now binds the controlled PCB-P0.9 native SHA-256 identity to P0.2 assembly data: sixteen exact-MPN lines totaling 42 populated references, 42 internal placements and four NPTH features. The item advances to `exact_candidate_hold`. Historical PCB-P0.5 CAM is prohibited from current use.

R150 adds current source-bound CAM review outputs without changing the `BOM-048` hold class or counts: ten Gerber/job files, five drill/map/report files, IPC-D-356, statistics, native DRC 0 and exact 42-reference internal position parity. The outputs are quarantined and not supplier-released. All eighteen manufacturing/release holds, eleven explicit manufacturing selections, supplier-normalized XYRS, provider/process acceptance, physical evidence and qualified review remain open. No contact, upload, quotation, fabrication, assembly, connection, motion or energization is authorized.

R151 advances `BOM-051` from `design_required` to `exact_candidate_hold` by binding native DXL-STAR-P0.1 to ten Gerber/job and five drill/map/report files, IPC-D-356, statistics, native DRC 0, exact parity for seven populated connector placements and eighteen terminal mappings, and four NPTH records. The package is quarantined and not supplier-released. The connector/current conflict, mating harness parts, conductors/crimps, protection, waveform, no-backfeed, grounding/shielding, thermal/load, supplier/process, first article, HIL/fault/EMC, qualified review and every work authorization remain open or false.

R152 resolves a duplicate allocation rather than selecting hardware. Electrical V3 has one `INJ1`; native DXL-STAR-P0.1 implements all three isolated VDD branches; all eighteen system/native terminal pairs match. `BOM-035` is therefore integrated in parent `BOM-051` with no separate purchase. Twelve residual hold groups preserve the complete board, connector, harness, protection, conductor/termination, current, waveform, no-backfeed, grounding, thermal, physical and qualified-review boundary.

R153 resolves a second duplicate allocation. Official ROBOTIS product pages list one assembled 180 mm JST-JST X3P cable in each held actuator package, so `BOM-086` records three integrated branch cables. `BOM-054` and `BOM-055` now allocate only two loose housings and four contacts for the one custom two-conductor controller cable. Fourteen residual holds preserve current, protection, routing, construction, physical-evidence and work-authority boundaries.
