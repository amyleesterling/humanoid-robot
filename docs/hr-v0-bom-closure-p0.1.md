...
R62 advances `BOM-019` to an exact-candidate hold for two Phoenix `PT 4-HESI (5X20)` item `3211861` holders and adds `BOM-072` for the two still-unresolved fuse links. R63 adds exact-candidate-hold `BOM-073` for one Phoenix `D-ST 4` item `3030420` end cover. Electrical V3-P1.7 and `HR-V0-CP-P0.3` carry the same holder/end-cover identity. Both fuse links, received accessory compatibility/grouping, all protection calculations and all physical evidence remain open; manufacturer maximum ratings are not project fuse ratings.
