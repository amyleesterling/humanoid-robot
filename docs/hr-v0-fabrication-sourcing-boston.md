...
Identifier: `HR-V0-FAB-SRC-P0.3`

Current geometry input: `HR-V0-ARM-ARCH-P0.6` / R67; part geometry is unchanged from the R66 sourcing screen

Controlled route register: `cad/hr-v0/manufacturing/hr-v0-r66-fabrication-route-register.csv`
...
The current R67 arm candidate uses:

- three `MV0-C01` standard adapter candidates and one H104-specific `MV0-C04`, each `48 x 40 x 9.525 mm` nominal;
...
- catalog ROBOTIS frames and current exact fastener candidates recorded in `HR-V0-ARM-ARCH-P0.6`.

The H104 and S102 source axes are explicit candidates, but received fit, T-slot capacity, complete cable/guard envelope, physical hard stop, stopping-overtravel/tolerance margin, bench anchor, supplier DFM, FAI, proof testing, and qualified mechanical acceptance remain open. The parent `HR-V0-MECH-P0.5` release hold remains in force.

## Recommended Boston-area route

Use catalog ROBOTIS/80/20 hardware wherever the current candidate permits it. For C01/C04/C05, obtain written DFM from a one-stop 3-axis CNC supplier only after a qualified reviewer accepts the P0.6 drawing controls. Use Artisans Asylum as the leading nearby candidate for machine-capability confirmation, training, minor finishing, or independent dimensional inspection. Use SendCutSend only as a possible profile-blank source followed by qualified secondary CNC machining; no current profile-only upload artifact exists, so nothing may be uploaded or ordered on that route.
...
|---|---:|---|---|---|
| `MV0-C01` adapter | 3 | One-stop 3-axis CNC from accepted drawing/STEP/DXF | Capability/DFM inquiry only after qualified drawing review | Material/MTR acceptance, supplier DFM, separate first article, FAI and proof |
...
| C01/C04/C05 profile blanks | 5 total | Profile cutting without holes/countersinks, then qualified secondary CNC | Research only; no controlled upload artifact exists | Separate hole-free artifacts, traceability, datum/fixture plan, finished-feature capability and FAI |
...
1. Qualified mechanical acceptance of the P0.6 C01/C04/C05 drawings, tolerances, material specification, provisional MTR threshold, load cases, and analytical method.
2. Received-fit closure of the H104, S102 and 40-4040 interfaces and complete adapter/support quantity/configuration.
