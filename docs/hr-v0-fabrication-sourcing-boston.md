...
Current geometry input: `HR-V0-ARM-ARCH-P0.5` / R66
...
The current R66 arm candidate uses:
...
- catalog ROBOTIS frames and current exact fastener candidates recorded in `HR-V0-ARM-ARCH-P0.5`.

The H104 and S102 source axes are explicit candidates, but received fit, T-slot capacity, complete cable/guard envelope, hard stop, stopping-overtravel margin, bench anchor, supplier DFM, FAI, proof testing, and qualified mechanical acceptance remain open. The parent `HR-V0-MECH-P0.4` release hold remains in force.

## Recommended Boston-area route

Use catalog ROBOTIS/80/20 hardware wherever the current candidate permits it. For C01/C04/C05, obtain written DFM from a one-stop 3-axis CNC supplier only after a qualified reviewer accepts the P0.5 drawing controls. Use Artisans Asylum as the leading nearby candidate for machine-capability confirmation, training, minor finishing, or independent dimensional inspection. Use SendCutSend only as a possible profile-blank source followed by qualified secondary CNC machining; no current profile-only upload artifact exists, so nothing may be uploaded or ordered on that route.
...
1. Qualified mechanical acceptance of the P0.5 C01/C04/C05 drawings, tolerances, material specification, provisional MTR threshold, load cases, and analytical method.
