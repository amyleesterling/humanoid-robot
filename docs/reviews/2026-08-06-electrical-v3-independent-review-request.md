# Project Button HR-V0 Electrical V3-P1.6 / PCB-P0.5 / DXL-STAR-P0.1 / HR-V0-CP-P0.2 Independent Review Request

Review date: 2026-08-07
Controlled candidate: **Electrical V3-P1.6 / PCB-P0.5 / DXL-STAR-P0.1 / HR-V0-CP-P0.2**
...
The authoritative source is this repository. The workshop website is presentation context only. Electrical V2.1 is the previously reviewed baseline; V3-P1.6 is a separate correction candidate and does not supersede it until its selections, calculations, tests, and qualified reviews close. Electrical P0.1 through P1.5 remain historical. P1.2 introduced `DXL-STAR-P0.1`; P1.3 retained the Schneider critical-current/application blocker; P1.4 added received-lot RESET/ARM terminal control; P1.5 froze exact amber H1 while preserving `TBD-HA/TBD-HB`; and P1.6 freezes only the Phoenix `3211861` FSR1/FSR2 holder candidates while leaving both fuse links, the end cover and all coordination unresolved. PCB-P0.5 and DXL-STAR-P0.1 are routed native candidates without fabrication outputs. HR-V0-CP-P0.2 supersedes the impossible P0.1 protection reserve with a larger Hammond enclosure/backplate and dimensioned device/service allocations; it releases no holes, cuts, wires, fuse links, protection values, bonds, cable entries or assembly. Supplier archive acceptance, physical test access, source current-sharing, harness/protection, connector-current, contactor critical-current/application disposition, thermal/waveform/no-backfeed, COM-slew, brownout/EMC, received proof, physical terminals, panel depth/heat/duct-fill/bonding/human factors, HIL and functional-safety blockers remain open.
...
- R62 protection-coordination evidence: `docs/hr-v0-protection-coordination-p0.2.md`; `electrical/hr-v0-protection-coordination-inputs.csv`; `tests/forms/hr-v0-protection-coordination-template.csv`
...
- R62 control-panel physical-definition evidence: `docs/hr-v0-control-panel-p0.2.md`; `electrical/panel/hr-v0-control-panel-p0.2/`; `tests/forms/hr-v0-control-panel-receiving-assembly-template.csv`
...
- HR-V0-CP-P0.2 with 23 panel-BOM rows, 20 nominal backplate allocations, five door rows, six exact XT1 positions, all 66 bounded V3 wire endpoints, six unreleased cable-entry zones, twelve thermal/space screens, 22 unexecuted evidence rows, and no released hole/cut/wire/fuse/protection/bond/entry/assembly value.
...
16. Audit HR-V0-CP-P0.2 against the physical devices and V3 source. Reproduce the P0.1 fit failure, then verify every P0.2 backplate/door allocation, received-fit/depth/service assumption, DIN-rail/duct/terminal compatibility, XT1 mapping, all 66 wire endpoints, cable-entry zones, operator legends, H1 diagnostic-only meaning, PCB envelopes, heat/loss/duty, duct fill, grounding/bonding, and every unexecuted evidence row. Independently verify Hammond `PJ242010RT` / `18P2117`, Phoenix `3211861`, Blue Sea `5025`, and Littelfuse `FHAC0002SXJ` dimensions. Confirm that allocations are not released hole coordinates and that no fuse link, drill, cut, conductor, termination, entry, PCB output, assembly or energization value is released.
