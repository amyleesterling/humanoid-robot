# Project Button HR-V0 Electrical V3-P1.8 / PCB-P0.5 / DXL-STAR-P0.1 / HR-V0-CP-P0.4 / HR-V0-SD-P0.2 Independent Review Request

Review date: 2026-08-07
Controlled candidate: **Electrical V3-P1.8 / PCB-P0.5 / DXL-STAR-P0.1 / HR-V0-CP-P0.4 / HR-V0-SD-P0.2**
...
The authoritative source is this repository. The workshop website is presentation context only. Electrical V2.1 is the previously reviewed baseline; V3-P1.8 is a separate correction candidate and does not supersede it until its selections, calculations, tests, and qualified reviews close. Electrical P0.1 through P1.7 remain historical. P1.2 introduced `DXL-STAR-P0.1`; P1.3 retained the Schneider critical-current/application blocker; P1.4 added received-lot RESET/ARM terminal control; P1.5 froze exact amber H1 while preserving `TBD-HA/TBD-HB`; P1.6 froze the Phoenix `3211861` FSR1/FSR2 holder candidates; P1.7 froze `D-ST 4` item `3030420` as the end-cover candidate; and P1.8 freezes active Littelfuse `75920-01` only as the exact SPST high-side SD1 catalog candidate. `TBD-IN/TBD-OUT`, conductor/lug, source fault, load-break, touch protection, cutout, zero-energy/padlock procedure, human factors and qualified Boston application review remain open, as do both FSR fuse links, received compatibility/grouping and all coordination. PCB-P0.5 and DXL-STAR-P0.1 are routed native candidates without fabrication outputs. HR-V0-CP-P0.4 retains the corrected larger Hammond allocation, adds one unreleased SD1 sidewall option, and releases no holes, cuts, wires, fuse links, protection values, bonds, cable entries or assembly. Supplier archive acceptance, physical test access, source current-sharing, harness/protection, connector-current, contactor critical-current/application disposition, thermal/waveform/no-backfeed, COM-slew, brownout/EMC, received proof, physical terminals, panel depth/heat/duct-fill/bonding/human factors, HIL and functional-safety blockers remain open.
...
- R64 protection-coordination evidence: `docs/hr-v0-protection-coordination-p0.4.md`; `electrical/hr-v0-protection-coordination-inputs.csv`; `tests/forms/hr-v0-protection-coordination-template.csv`
...
- R64 control-panel and service-disconnect evidence: `docs/hr-v0-control-panel-p0.4.md`; `electrical/panel/hr-v0-control-panel-p0.4/`; `docs/hr-v0-service-disconnect-p0.2.md`; `tests/forms/hr-v0-control-panel-receiving-assembly-template.csv`; `tests/forms/hr-v0-service-disconnect-receiving-application-template.csv`
- R61 H1 received-evidence route: `docs/hr-v0-h1-receiving-p0.1.md`; `tests/forms/hr-v0-h1-receiving-template.csv`
...
- Root schematic plus twelve child sheets: `electrical/kicad/project-button-v3/*.kicad_sch`
...
- HR-V0-CP-P0.4 with 25 panel-BOM rows, 20 nominal backplate allocations, five door rows, one fail-closed sidewall option, six exact XT1 positions, all 66 bounded V3 wire endpoints, six unreleased cable-entry zones, twelve thermal/space screens, 22 unexecuted panel rows plus 15 unexecuted SD1 rows, and no released hole/cut/wire/fuse/lockout/protection/bond/entry/assembly value.
...
16. Audit HR-V0-CP-P0.4 against the physical devices and V3 source. Reproduce the P0.1 fit failure, then verify every P0.4 allocation, all 66 wire endpoints, cable-entry zones, H1 meaning, PCB envelopes, heat/loss/duty, duct fill, grounding/bonding, and every unexecuted evidence row. Independently verify Hammond `PJ242010RT` / `18P2117`, Phoenix `3211861` plus `3030420`, Blue Sea `5025`, Littelfuse `FHAC0002SXJ`, and exact SD1 candidate `75920-01`. Audit `HR-V0-SD-P0.2`: reproduce the Blue Sea `6004200` and ABB `OTDCP25SA11M` rejection logic, verify 75920 Series datasheet Rev 091825, exact 2D print and IF-165 Rev 010320-C, confirm `TBD-IN/TBD-OUT` are not inferred stud designations, and identify every conductor/lug, source fault, load-break, touch-protection, cutout, zero-energy/padlock, legend/human-factors and Boston application hold. Confirm that no fuse link, SD1 cutout, lockout procedure, drill, cut, conductor, termination, entry, PCB output, assembly or energization value is released.
17. Independently verify H1 exact-order-code facts against the current IDEC page and 2026-07-23 screw-terminal catalog. Confirm the schematic and physical schedule use `TBD-HA/TBD-HB` only as project placeholders, never manufacturer terminal or polarity claims. Review all fourteen H1 receiving/characterization records, powered-component-test authorization boundary, measurement inputs, diagnostic-only legend, failure interpretations and later disconnected-load HIL requirement. Confirm H1 cannot be interpreted as safe, armed, motion enabled or entry permission.
18. Inspect the PDF and every SVG, including `panel-layout.svg`, at normal viewing size for clipping, overlap, tiny text, ambiguous crossings, misleading contact state, incomplete titles, or absent preliminary warnings. Treat web rendering as a separate presentation check.
