# Project Button HR-V0 Electrical V3-P1.5 / PCB-P0.5 / DXL-STAR-P0.1 / HR-V0-CP-P0.1 Independent Review Request

Review date: 2026-08-07
Controlled candidate: **Electrical V3-P1.5 / PCB-P0.5 / DXL-STAR-P0.1 / HR-V0-CP-P0.1**
...
The authoritative source is this repository. The workshop website is presentation context only. Electrical V2.1 is the previously reviewed baseline; V3-P1.5 is a separate correction candidate and does not supersede it until its selections, calculations, tests, and qualified reviews close. Electrical P0.1 through P1.4 remain historical. P1.2 retained the watchdog PCB boundary and test points while replacing three undefined inline DYNAMIXEL injection modules with one exact central `DXL-STAR-P0.1` board boundary. P1.3 adds current Schneider K1/K2 catalog evidence and explicitly retains the lower-current critical-current/application blocker. P1.4 adds the IDEC received-lot terminal-map control while retaining all four S1/S2 terminals as `TBD-*`. P1.5 freezes H1 as exact amber IDEC `HW1P-1FQD-A-24V`, removes misleading `SAFE ELIGIBLE` and `+/-` wording, and preserves `TBD-HA/TBD-HB` project placeholders until the fourteen-row received/characterization route closes. PCB-P0.5 is a two-layer routed/test-access candidate with 201 segments, 56 vias, one filled return zone, two separate floating 2 mm x 2 mm SUB thermal zones, native DRC 0/0 and independent pad/copper-connectivity evidence. DXL-STAR-P0.1 is a separate 100 mm x 60 mm, two-layer routed candidate with seven connectors, 18 terminals, 17 segments, one common-return zone, mutually isolated J1/J2/J3 VDD nets and an unrouted U2D2 pin 2. Neither project has fabrication outputs. HR-V0-CP-P0.1 is a physical panel-allocation candidate with exact catalog enclosure/backplate, terminal/rail/duct and amber H1 candidates on hold; it releases no holes, cuts, wires, protection values, bonds, cable entries or assembly. Supplier archive acceptance, physical test access, source current-sharing, harness/protection, connector-current, contactor critical-current/application disposition, thermal/waveform/no-backfeed, COM-slew, brownout/EMC, received proof, physical terminals, panel depth/heat/duct-fill/bonding/human factors, HIL and functional-safety blockers remain open.
...
- R60 control-panel physical-definition evidence: `docs/hr-v0-control-panel-p0.1.md`; `electrical/panel/hr-v0-control-panel-p0.1/`; `tests/forms/hr-v0-control-panel-receiving-assembly-template.csv`
...
- HR-V0-CP-P0.1 with 20 panel-BOM rows, 16 nominal backplate allocations, five door rows, six exact XT1 positions, all 66 bounded V3 wire endpoints, six unreleased cable-entry zones, ten thermal/space screens, 20 unexecuted evidence rows, and no released hole/cut/wire/protection/bond/entry/assembly value.
...
16. Audit HR-V0-CP-P0.1 against the physical devices and V3 source. Verify every backplate/door coordinate, received-fit/depth/service assumptions, DIN-rail/duct/terminal compatibility, XT1 part/color/order and exact net mapping, all 66 copied wire endpoints, door-flex and cable-entry zones, operator spacing/legends, H1 amber diagnostic-only meaning, PCB mounting/service envelopes, component heat/loss/duty, duct fill, grounding/bonding, enclosure rating after penetrations, and every unexecuted evidence row. Confirm that the reserved lower zone is not represented as proven adequate. Independently derive conductor, termination, gland and protection inputs without inventing selections. Confirm that no drill, cut, conductor, termination, entry, PCB output, assembly or energization value is released.
