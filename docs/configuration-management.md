...
- **Project Button Electrical V3-P0.5** identifies the current R26 correction candidate. It retains P0.4's pin-level `ISO1212DBQ` circuit and calculated threshold, contact-wetting and logic-interface networks, freezes IDEC `HW1B-M1F10-B` black RESET and `HW1B-M1F10-G` green ARM operator identities, and freezes the official Raspberry Pi 27W US regional model without inferring an unmapped SKU. Exact received terminals, panel human factors, compute-supply SKU/color/retention, passive order codes, PCB, EMC, physical tests and qualified review remain open. `P0.5` has not superseded V2.1 and is not a released electrical baseline.
