...
- **HR-V0-MECH-P0.3** identifies the current mechanical correction hold. R53 uses exact manufacturer-coordinate ROBOTIS STEP files under **HR-V0-ROBOTIS-IF-P0.1**, withdraws the P0.2 J1/J2/G1 chain and `MV0-001` through `MV0-003`, and replaces **HR-V0-FAB-RFI-P0.1** with **HR-V0-FAB-RFI-P0.2-WITHDRAWN** containing zero supplier ZIPs. The base cuts and **HR-V0-FRAME-P0.2** remain candidate coordination evidence, but the arm has no buildable geometry. `MECH-005` / `AUDIT-MECH-012` require a replacement exact-coordinate assembly, parallel-axis proof, collision/tool-access/cable study, load path, fasteners, tolerances, FAI and qualified disposition before any arm quotation or fabrication route can reactivate.
