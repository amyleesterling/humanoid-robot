...
- **Project Button Electrical V3-P0.6** identifies the current R27 correction candidate. It retains P0.4's pin-level `ISO1212DBQ` circuit and P0.5's operator/compute-source identities, then freezes the XW E-stop's right and left 2NC contact positions as project-unique terminals. Received S0 verification, RESET/ARM terminals during IDEC's active old/new production transition, panel human factors, compute-supply SKU/color/retention, passive order codes, PCB, EMC, physical tests and qualified review remain open. `P0.6` has not superseded V2.1 and is not a released electrical baseline.
