...
- **Project Button Electrical V3-P0.7** identifies the current R28 correction candidate. It retains P0.4's pin-level `ISO1212DBQ` circuit, P0.5's operator/compute-source identities, and P0.6's XW E-stop contact-position allocation. P0.7 freezes a project-side Molex `39012066`/`444783112` HCS connector candidate for `JA1` and a TRACO POWER `TSR 1-2450` watchdog regulator with exact pins. Source-side contact construction, harness current sharing, wire/protection/distribution, thermal/current-division evidence, converter brownout/EMC behavior, received terminals, panel human factors, passive order codes, PCB, physical tests and qualified review remain open. `P0.7` has not superseded V2.1 and is not a released electrical baseline.
