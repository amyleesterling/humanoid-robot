# HR-V0 control-panel physical-definition candidate P0.3

**PRELIMINARY - NOT APPROVED FOR PROCUREMENT, FABRICATION, ASSEMBLY, OR ENERGIZATION**

Document ID: `HR-V0-CP-P0.3`

Date: 2026-08-07

Electrical input: `Project Button Electrical V3-P1.7`

Supersedes: `HR-V0-CP-P0.2` as the current layout candidate; P0.1 and P0.2 remain configuration history.
...
P0.3 retains P0.2's corrected enclosure candidate, Hammond `PJ242010RT`, and white steel inner panel `18P2117`. Hammond publishes an enclosure outside size of `610 x 508 x 257 mm` and an inner-panel size of `21 x 17 in` (`533.4 x 431.8 mm`). The enlarged nominal panel allows distinct planning envelopes for the branch fuse block, two safety-output fuse holders, the inline main-fuse holder, and a still-visible unresolved inlet/disconnect/fuse-link reserve.

P0.3 adds the exact Phoenix Contact `D-ST 4`, item `3030420`, end-cover candidate for the FSR holder group. Phoenix publishes a width of `2.2 mm`, height of `55.9 mm`, and depth of `29 mm`. With two `6.2 mm`-wide holders, the nominal body-plus-end-cover width is `14.6 mm`; the existing `25 x 75 mm` planning envelope therefore retains lateral planning allowance. This arithmetic does not prove received compatibility, terminal access, conductor bends, fuse service, grouping temperature, or touch protection.
...
- `electrical/panel/hr-v0-control-panel-p0.3/panel-bom.csv`: 24 current candidate/selection rows.
...
- `thermal-space-screen.csv`: twelve bounded screens, including the P0.1 fit defect and P0.3 protection allocations.
...
Every P0.3 rectangle fits inside the nominal `431.8 x 533.4 mm` panel boundary. The bottom allocation is:
...
1. Select and coordinate all six fuse links, `JC1`, `SD1`, conductors, terminations, glands, and bonding hardware; receive and verify the candidate end cover.
