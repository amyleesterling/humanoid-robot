...
| `SF-02` control-loss/watchdog stop | Stale or uncontrolled motion after supervisor loss; `SAFE-003`, `CTRL-003`, `WALK-008`, R-003/R-020 | Remove actuator-rail energy within the released heartbeat, dropout, and clearance budget | Independent heartbeat monitor and hardware restart interlock; two series contactors are the final energy-removal elements | Heartbeat restoration, reboot, or software fault clearing cannot restore either coil. Physical reset and a later distinct `ARM` are both required | **ANALYSIS REQUIRED** | **BLOCKER:** V2.1 downstream permit can reclose on heartbeat restoration. Exact hardware restart interlock is **SELECTION REQUIRED**; firmware latch is not credited |
| `SF-03` prevention of unexpected restart and external-device monitoring | Motion or re-energization during reset/maintenance; `SAFE-008`, `ELEC-002`, R-001/R-030 | Keep contactors and torque inhibited when feedback is inconsistent, a contactor is welded, reset is invalid, or state/order is wrong | Falling-edge reset path, K1/K2 normally closed mirror contacts, hardware post-watchdog restart inhibit, and deterministic arm/drive state machine | Cause removal -> EDM healthy -> accepted physical reset -> `SAFE_READY` with coils inhibited -> distinct `ARM` -> fresh command; no stale target resumes | **ANALYSIS REQUIRED** | Start/EDM circuit is preliminary. Protected routing, fault exclusions, mirror-contact validity, and hardware arm/restart gating are open |
...
- Replace the Electrical V2.1 downstream watchdog permit with a selected, documented hardware architecture that prevents heartbeat restoration from restoring K1/K2 or KH1/KH2 before physical reset and distinct `ARM`.
