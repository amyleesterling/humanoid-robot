...
That V2.1 downstream watchdog-permit position remains a **BLOCKER** for V2.1. The corrected native V3-P0.6 source routes one watchdog contact through each SR1 input return. A watchdog dropout therefore requires a new monitored RESET at SR1 and then a separate monitored ARM at SRA1. P0.6 retains P0.4's calculated dual-channel `ISO1212DBQ` circuit, P0.5's distinct black RESET and green ARM operator identities, and project-unique right/left terminal designators for the XW E-stop's two manufacturer 1-2 NC pairs. Received S0 verification, RESET/ARM terminal mapping during IDEC's active old/new assembly transition, and panel human factors remain open. The candidate uses 1 kOhm `RTHR`, 562 Ohm `RSENSE`, 10 nF `CIN`, a 2.70 kOhm 0.5 W wetting load, 1 kOhm output-series resistor and 10 kOhm pulldown per channel. Exact passive order codes, PCB, remaining terminals, EMC, thermal, fault injection and HIL remain open. This is a connected design candidate, not proof: the ordinary RP2040 controller and Phoenix Contact relays receive no PL/SIL credit by assertion, a welded watchdog contact can defeat the heartbeat-stop channel, shared supply/controller/clock/firmware failures remain common-cause concerns, and the topology still requires FMEA, qualified review, and `TEST-SAFE-002` fault traces.
