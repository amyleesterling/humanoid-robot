...
That V2.1 downstream watchdog-permit position remains a **BLOCKER** for V2.1. The corrected native V3-P0.7 source routes one watchdog contact through each SR1 input return. A watchdog dropout therefore requires a new monitored RESET at SR1 and then a separate monitored ARM at SRA1. P0.7 retains P0.4's calculated dual-channel `ISO1212DBQ` circuit, P0.5's distinct black RESET and green ARM operator identities, P0.6's project-unique right/left XW E-stop terminal designators, and adds exact project-side JA1 connector and DC1 regulator candidates. Received S0 verification, RESET/ARM terminal mapping during IDEC's active old/new assembly transition, panel human factors, current-division/thermal proof and converter brownout evidence remain open. The candidate uses 1 kOhm `RTHR`, 562 Ohm `RSENSE`, 10 nF `CIN`, a 2.70 kOhm 0.5 W wetting load, 1 kOhm output-series resistor and 10 kOhm pulldown per channel. Exact passive order codes, PCB, remaining terminals, protection/conductors, EMC, thermal, fault injection and HIL remain open. This is a connected design candidate, not proof: the ordinary RP2040 controller and Phoenix Contact relays receive no PL/SIL credit by assertion, a welded watchdog contact can defeat the heartbeat-stop channel, shared supply/controller/clock/firmware failures remain common-cause concerns, and the topology still requires FMEA, qualified review, and `TEST-SAFE-002` fault traces.
