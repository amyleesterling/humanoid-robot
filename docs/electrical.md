...
Current R87 correction: Electrical V3-P1.13 connects each S0 NC channel directly from SR1 and back to SR1. KWD1/KWD2 no longer appear in either input return; their NO contacts series-gate only `SR1:A1`. This removes the source-encoded KWD-to-E-stop-return injection path found in R86. Earlier P1.12 records remain historical configurations and are superseded for current guidance by `docs/hr-v0-watchdog-supply-gate-p0.1.md`. Physical noninterference, contact application, routing, recovery, fault injection and qualified functional-safety allocation remain open.
...
The controlled [PNOZ s4 operating manual 21396-EN-23](https://www.pilz.com/download/open/OM_PNOZ_s4_21396-EN-23.pdf) has PDF metadata dated 2026-06-17 and a Pilz portal file date of 2026-06-22; the exact acquired file was hashed and rechecked on 2026-08-08. It supports the proposed short-across-contact detection and monitored falling-edge start mode. In current V3-P1.13, `SR1:S11-S12` and `SR1:S21-S22` each contain only one positively opening S0 NC contact; no watchdog contact appears in either return. `SR1:S12 -> S1 RESET -> SR1:S34` implements the monitored RESET path. SR1 safety outputs `13-14` and `23-24` separately gate the two SRA1 input channels. `SRA1:S12 -> S2 ARM -> K1:21-22 -> K2:21-22 -> SRA1:S34` implements the later distinct ARM/EDM path. Separate SRA1 safety outputs feed the still-unreleased K1 and K2 coil-protection paths. The exact source-to-net mapping is controlled in `docs/hr-v0-pnoz-path-conformance-p0.1.md`; it is not Pilz application approval or functional-safety validation.
...
That V2.1 downstream watchdog-permit position remains a **BLOCKER** for V2.1. The corrected native V3-P1.13 source instead places two ordinary watchdog NO contacts in series with `SR1:A1` only. A watchdog dropout removes SR1 supply eligibility; after heartbeat recovery, SR1 still requires a new monitored RESET and SRA1 still requires a later distinct monitored ARM through K1/K2 EDM. P1.8 retains the exact Phoenix `3211861` FSR1/FSR2 holders and `D-ST 4` item `3030420` group end-cover candidate, and freezes active Littelfuse `75920-01` only as the exact SPST high-side SD1 catalog candidate. P1.12 removes the invented installed debug connector, freezes Pi GPIO17/header pin 11 and header pin 6 return, and limits watchdog debug access to exact TP15/TP16/TP2 test points. `TBD-HA/TBD-HB`, `SD1:TBD-IN/TBD-OUT`, both FSR fuse links, received accessory compatibility/grouping, SD1 conductor/fault/load-break/touch-protection/lockout application and all protection coordination remain unresolved. H1 and the heartbeat/debug path are diagnostic/ordinary-control only and have no safety credit or motion authority. PCB-P0.6 and DXL-STAR-P0.1 are routed and DRC-clean source candidates, but neither is a fabrication release. R64 adds `HR-V0-CP-P0.4` and `HR-V0-SD-P0.2`; it releases no drilling, cutting, conductors, protection values, lockout procedure, PE bond, glands, assembly or PCB fabrication. Received-device proof, cable construction, GPIO runtime/startup/timing, debug fixture/no-back-power proof, connector-current application, passive measurements/derating, RESET/ARM/H1 terminals, panel depth/heat/duct-fill/bonding/human factors, source current division, converter brownout, supplier acceptance, physical test access, COM-slew, protection/conductors, no-backfeed, signal integrity, EMC, thermal, fault injection and HIL remain open. R44 classifies this ordinary heartbeat path as `DF-01` with **NO SAFETY CREDIT**: welded contacts and shared controller/supply/clock/firmware failures are assumed possible. Physical routing and fault analysis must still prove that `DF-01` cannot impair the separately credited-candidate `SF-01` E-stop or `SF-03` restart-prevention paths. See `docs/hr-v0-functional-safety-allocation-p0.1.md`.

Required restart sequence:

`heartbeat invalid -> KWD supply gate removes SR1 eligibility -> SR1 and SRA1 drop -> K1 and K2 drop -> EDM proves dropped -> heartbeat may become healthy but coils stay inhibited -> valid falling-edge physical RESET -> SR1 eligible with coils still inhibited -> distinct falling-edge physical ARM through EDM -> contactors may energize -> fresh trajectory validation -> torque/motion`
