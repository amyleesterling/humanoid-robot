...
def write_docs(sheets):
    ...
    panels = "".join(
        f'<details><summary>{sheet.number:02d} · {html.escape(sheet.title)}</summary>'
        f'<div class="drawing"><object data="output/{html.escape(f"{PROJECT}-{sheet.number:02d} {sheet.title}.svg", quote=True)}" '
        f'type="image/svg+xml" aria-label="{html.escape(sheet.title, quote=True)}"></object></div></details>'
        for sheet in sheets
    )
    axis = axis_rows()
