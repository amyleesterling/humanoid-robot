...
SOURCES = {
    ...
    "GV12": ("Sensata / GIGAVAC", "GV121CAC", "GV12 series Rev 28-Apr-2022", "https://www.sensata.com/sites/default/files/a/sensata-gigavac-gv12-series-100v-contactors-datasheet.pdf"),
...
def build_sheets(model):
    ...
    s3 = model.Sheet(3, "03_redundant_dc_interruption.kicad_sch", "Two series GV121CAC contactors and feedback loop", "External-panel DC interruption candidate; no safety-performance claim.")
    s3.components = [
        comp(model, "K1", "SENSATA GIGAVAC GV121CAC", [("M1", "MAIN INPUT STUD", "RAW_12V_POS", "left"), ("M2", "MAIN OUTPUT STUD", "K1_OUT", "right"), ("COIL+", "24 V COIL POSITIVE", "K1_COIL_POS", "left"), ("COIL-", "COIL RETURN", "SAFE_0V", "left"), ("AUX-C", "NC AUX COMMON", "RESET_EDM", "left"), ("AUX-NC", "NC AUX RETURN", "EDM_K1_OUT", "right")], "Ordering code decodes to upright mounting, 24 V internally suppressed coil, flying leads and SPST-NC mechanically linked auxiliary. Main M8 stud torque, wire colors/identification, DC interruption duty and safety suitability require received-device and qualified verification.", (135, 110), 104, SOURCES["GV12"][3], SOURCES["GV12"][2], "EXACT ORDER-CODE CANDIDATE - NOT AN IEC MIRROR-CONTACT CLAIM"),
        comp(model, "K2", "SENSATA GIGAVAC GV121CAC", [("M1", "MAIN INPUT STUD", "K1_OUT", "left"), ("M2", "MAIN OUTPUT STUD", "TETHER_POS_SWITCHED", "right"), ("COIL+", "24 V COIL POSITIVE", "K2_COIL_POS", "left"), ("COIL-", "COIL RETURN", "SAFE_0V", "left"), ("AUX-C", "NC AUX COMMON", "EDM_K1_OUT", "left"), ("AUX-NC", "NC AUX RETURN", "RESET_EDM", "right")], "Second independent series switching element. Common-cause, welded-contact detection, coil suppression behavior, contact protection, fault current, life and stopping time remain unvalidated.", (320, 110), 104, SOURCES["GV12"][3], SOURCES["GV12"][2], "EXACT ORDER-CODE CANDIDATE - APPLICATION OPEN"),
    ...
    s3.notes = ["Both high-current contactors are in the external panel, not on the robot.", "The GV12 auxiliary is manufacturer-described as mechanically linked; this package does not call it an IEC mirror contact or claim a safety category."]
    sheets.append(s3)
...
def write_cad():
    ...
    k1 = rounded_box(70, 62, 82, (70, -25, 150), 5)
    k2 = rounded_box(70, 62, 82, (155, -25, 150), 5)
    ...
    panel_parts = [(shell, "ENCLOSURE_1418N4C6", (0.65, 0.72, 0.78, 0.35)), (backplate, "BACK_PANEL_432x330", (0.75, 0.78, 0.80, 1)), (rsp, "PS1_RSP-500-12", (0.18, 0.45, 0.78, 1)), (din, "DIN_RAIL", (0.72, 0.72, 0.72, 1)), (pnoz, "SR1_PNOZ_s4_750104", (0.95, 0.75, 0.12, 1)), (sd15, "PS2_SD-15A-24", (0.22, 0.65, 0.85, 1)), (k1, "K1_GV121CAC", (0.74, 0.15, 0.13, 1)), (k2, "K2_GV121CAC", (0.74, 0.15, 0.13, 1)), (estop, "S0_ESTOP_FAMILY", (0.86, 0.05, 0.05, 1)), (reset, "S1_RESET", (0.95, 0.76, 0.12, 1)), (tether, "XT1A_SBS75G", (0.12, 0.18, 0.24, 1))]
...
def write_docs(sheets):
    ...
    for key, (maker, part, rev, url) in SOURCES.items():
        source_rows.append({"source_id": key, "manufacturer": maker, "candidate": part, "document_revision_or_date": rev, "accessed": DATE, "official_url": url, "verified_scope": {
            ...
            "GV12": "GV121CAC ordering decode; 24 V internally suppressed coil; flying leads; NC mechanically linked auxiliary; M8 main studs",
    ...
    holds = [
        ...
        ("H04", "GV121CAC received-terminal identification, DC interruption duty, auxiliary suitability, fault current, life and common-cause analysis"),
