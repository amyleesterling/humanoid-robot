...
DRIVES = (
    Drive("LD-15", "JMF-07-LEG-REDUCED-15", 20, 30, 45, 10.0, "GPA20GT5090-A-H10", "GPA30GT5090-A-H12", "GBN225EV5GT-090", "XH540", "HN13-N101 / SKU 903-0276-000", ("L_HIP_PITCH", "R_HIP_PITCH")),
    Drive("LD-20", "JMF-08/JMF-09-LEG-REDUCED-20", 20, 40, 51, 10.0, "GPA20GT5090-A-H10", "GPA40GT5090-A-H12", "GBN255EV5GT-090", "XH540 or XM430 by axis", "HN13-N101 / SKU 903-0276-000 or HN12-N101 / SKU 903-0238-000", ("L_HIP_ROLL", "R_HIP_ROLL", "L_KNEE_PITCH", "R_KNEE_PITCH", "L_ANKLE_ROLL", "R_ANKLE_ROLL")),
    Drive("LD-25", "JMF-10-ANKLE-PITCH-REDUCED-25", 16, 40, 50, 8.0, "GPA16GT5090-A-H8", "GPA40GT5090-A-H12", "GBN250EV5GT-090", "XM430", "HN12-N101 / SKU 903-0238-000", ("L_ANKLE_PITCH", "R_ANKLE_PITCH")),
)

PULLEY_OD_MM = {16: 24.32, 20: 30.69, 30: 46.61, 40: 62.52}
PULLEY_FLANGE_OD_MM = {16: 29.0, 20: 35.0, 30: 51.0, 40: 67.0}
...
def integrate_root() -> None:
    ...
    status.update({
        ...
        "reduced_leg_drivetrain_module_count": 3,
        ...
        "reduced_leg_drivetrain_horn_adapters_complete": False,
    ...
    section = f'''{start}<section id="leg-drivetrains"><h2>The ten reduced leg axes now use three concrete belt-module candidates</h2><div class="grid"><article class="card pass"><div class="metric">10 axes</div><p>Hip pitch/roll, knee pitch and ankle pitch/roll are allocated bilaterally.</p></article><article class="card pass"><div class="metric">3 modules</div><p>1.5:1, 2.0:1 and 2.5:1 candidates have exact tooth counts, belt lengths and solved centers.</p></article><article class="card pass"><h3>Purchasable families</h3><p>MISUMI 5GT pulleys and EV5GT belts plus ROBOTIS X540/X430 horn families are named.</p></article><article class="card hold"><h3>Capacity remains open</h3><p>Horn adapters, tension, tooth loads, shafts, fits, guards, fatigue and physical proof are unresolved.</p></article></div><p><a href="leg-drivetrain-p0.1/index.html">Open the leg-drivetrain guide</a> · <a href="leg-drivetrain-p0.1/axis-drivetrain-allocation.csv">axis allocation</a> · <a href="leg-drivetrain-p0.1/belt-center-geometry.csv">center geometry</a>.</p></section>{end}'''
    ...
    for hold in holds:
        if hold["hold_id"] == "HR30-P01-H03":
            hold["unresolved_item"] = (
                ...
                "hip/knee/ankle axes now map to three exact MISUMI 5GT/EV5GT product candidates with solved "
...
def main() -> int:
    ...
    for index, drive in enumerate(DRIVES):
        ...
        shifted = compound.translate(((index - 1) * 105.0, 0.0, 0.0))
        lineup_shapes.append(shifted)
        lineup.add(
            assembly,
            name=drive.drive_id,
            loc=cq.Location(cq.Vector((index - 1) * 105.0, 0.0, 0.0)),
        )
    ...
    for drive in DRIVES:
        for axis in drive.axis_ids:
            ...
            axis_rows.append({"axis_id": axis, "drive_id": drive.drive_id, "ratio": f"{drive.output_teeth / drive.motor_teeth:.3f}:1", "motor_pulley": drive.motor_pulley_code, "output_pulley": drive.output_pulley_code, "belt": drive.belt_code, "actuator_horn_family": horn, "horn_to_pulley_adapter": "PROJECT CUSTOM ADAPTER - DIMENSIONS/FASTENERS/PROOF SELECTION REQUIRED", "release_state": "PRODUCT/GEOMETRY CANDIDATE - CAPACITY AND PHYSICAL VALIDATION OPEN", "warning": WARNING})
    ...
    status = {"identifier": IDENTIFIER, "module_count": 3, "axis_count": len(axis_rows), "candidate_product_count": len(product_rows), "native_step_module_count": 3, "native_glb_module_count": 3, "lineup_step_present": True, "lineup_glb_present": True, "belt_pitch_length_closure_error_mm_max": max(float(row["length_closure_error_mm"]) for row in values), "exact_candidate_product_allocation_present": True, "vendor_tooth_brep_present": False, "horn_adapter_complete": False, "capacity_validated": False, "tension_validated": False, "physical_validation_complete": False, "procurement_authority": False, "fabrication_authority": False, "assembly_authority": False, "powered_test_authority": False, "motion_authority": False, "energization_authority": False, "warning": WARNING}
    ...
    (OUT / "README.md").write_text(f"# HR-30 reduced-leg drivetrain P0.1\n\n**{WARNING}**\n\nThree product-specific 5GT/EV5GT modules cover all ten reduced leg axes. Native CAD represents catalog external/routing envelopes and exact solved pitch centers; it does not claim vendor tooth B-Reps, capacity, fit, horn-adapter completion or work authority.\n", encoding="utf-8", newline="\n")
