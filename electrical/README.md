...
## Electrical V3-P1.12 correction candidate
...
Current generated counts are 76 component blocks, 296 modeled terminals, 103 native nets (64 named connected nets plus 39 deliberate auto-generated unconnected nets), 257 unique wire labels, 64 unresolved component/interface rows, and 10 deliberately unresolved `TBD-*` terminal designations. P1.2 replaced three undefined inline injection modules with one exact 18-terminal `INJ1` representation synchronized to the separate native `DXL-STAR-P0.1` project; P1.3 corrected the K1/K2 application record; P1.4 retains received-lot terminal control for RESET and ARM without inventing four terminal marks; P1.5 freezes exact amber H1 while replacing misleading `SAFE ELIGIBLE` and `+/-` labels with diagnostic-only wording and `TBD-HA/TBD-HB` project placeholders. P1.11 replaces the unsupported P1J conversion chain with the exact GlobTek `WR9QI1660YL4NKITR6B` factory-cord boundary and keeps `J24`, `F24`, received fit, protection and physical evidence on hold. P1.12 removes the invented installed `JDBG1`, freezes Pi GPIO17/header pin 11 and header pin 6 return, and limits watchdog debug access to TP15/TP16/TP2 while the cable, runtime, fixture and physical evidence remain open. U2D2 VDD is omitted; three actuator-positive rails remain isolated; common TTL data and return are explicit. The routed star board passes native ERC/DRC 0/0, but fabrication, harness, protection, connector-current, thermal, waveform, no-backfeed, received H1 evidence and qualified review remain open. KiCad 10.0.5 ERC reports 0 errors and 0 warnings; native netlist, thirteen-page PDF, and thirteen SVG exports succeed. Run:
