...
- heartbeat recovery may restore only the watchdog relay commands; Electrical V3-P0.8 requires a later physical SR1 RESET and then a distinct physical SRA1 ARM before K1/K2 can return;
...
This module cannot establish a safety category, PL or SIL. Both channels share the same MCU, power, clock, code and configuration. The P0.4 interface candidate freezes GP2/physical pin 4 for heartbeat, GP3/5 and GP4/6 for the two drives, and GP6/9 and GP7/10 for conditioned NC feedback. Electrical V3-P0.8 adds an exact `VO618A-4X017T` heartbeat interface with 910 ohm input resistor and 10 kilohm pullup, plus two separate `TPL7407LPWR` driver packages. The ISO1212 circuit is calculated but its PCB and exact passives remain open; every circuit still requires polarity, timing, COM-slew, EMC, brownout, fault-injection, clock-supervision, platform-startup, build-toolchain and HIL evidence under `TEST-ELEC-005`.
