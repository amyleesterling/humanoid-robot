#!/usr/bin/env python3
"""Generate dimensioned whole-body actuator-power route guides for HR-30.

The package turns the six straight corridor reservations into actual tangent
centerlines with two circular end turns.  It is geometry evidence only: the
external guarded-spine concept still needs motion-envelope, snag, clamp,
thermal, flex-life and physical validation before any fabrication or power.
"""
...
def route_geometry() -> tuple[list[dict[str, object]], dict[str, dict[str, object]]]:
    ...
    for segment_id in sorted(selected):
        ...
        rows.append(common({
            ...
            "route_class": "EXTERNAL GUARDED SPINE CANDIDATE",
            "collision_state": "NOT EXECUTED - FULL POSE, FALL, RESTRAINT AND HUMAN-CONTACT ENVELOPES OPEN",
            "state": "DIMENSIONED ROUTE-GUIDE CANDIDATE; CUT LENGTH, CLAMPS, GUARD AND MOTION PROOF OPEN",
        }))
...
def guard_rows(route_rows: list[dict[str, object]]) -> list[dict[str, object]]:
    return [common({
        ...
        "material_candidate": "removable rounded polymer shell over structural standoffs; exact material SELECTION REQUIRED",
        "opening_rule": "no reachable pinch or snag edge in any validated pose; no cable used as a structural stop",
        "service_rule": "tool-accessible removable cover; cable and clamp visually inspectable",
        "validation": "whole-body collision sweep; fall-restraint sweep; dress rehearsal; chafe/pull/impact inspection",
        "state": "ENVELOPE INTENT ONLY - GUARD SOLID NOT RELEASED",
