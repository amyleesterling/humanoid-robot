...
def main() -> int:
    ...
    for bus in buses:
        ...
        field_housing = "JST GHR-03V-S" if bus["protocol"].startswith("RS-485") else "JST GHR-02V-S"
        ...
        boundaries.append({"boundary_id": f"CB-{bus_id}", "boundary_type": "CONTROLLER / SEGMENT DATA-ONLY", "bus_id": bus_id, "axis_id": "N/A", "known_side": f"Carrier {pinout['carrier']} {pinout['interface_device']} / {pinout['stm32_peripheral']}", "known_pin_mapping": field_header, "mating_part": f"{field_housing} WITH JST SSHL-002T-P0.2 CONTACTS; ASSEMBLED CABLE SELECTION REQUIRED", "validation": "SOURCE-VERIFIED CANDIDATE; PCB/CABLE/EMC/FAULT TEST NOT EXECUTED"})
    ...
    source_rows += [
        ...
        {"source_id": "JST-GH", "official_url": JST_GH_SOURCE, "revision_or_date": "CURRENT OFFICIAL CATALOG; REVISION DATE NOT STATED", "accessed_date": ACCESSED, "verified": "GHR-02V-S/GHR-03V-S/GHR-15V-S housings; BM02B/BM03B/BM15B-GHS-TBT headers; SSHL-002T-P0.2 contact family", "not_verified": "assembled cable, flex life, retention, environmental and EMC performance"},
