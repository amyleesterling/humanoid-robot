...
def main() -> int:
    ...
    carrier_pinout = read_csv(PKG / "electrical" / "kicad" / "hr30-whole-body-electrical-p0.1" / "interface-carrier-pinout.csv")
    if len(buses) != 8 or len(axes) != 25 or len(routes) != 12:
        raise SystemExit("whole-body bus/axis/route inputs are incomplete")
    if len(carrier_pinout) != 8 or {row["bus_id"] for row in carrier_pinout} != {row["bus_id"] for row in buses}:
        raise SystemExit("eight-channel physical carrier pinout is incomplete")
    ...
    for bus in buses:
        ...
        field_header = pinout["field_header"]
        field_housing = "PAP-03V-S / SPHD-001T-P0.5" if bus["protocol"].startswith("RS-485") else "PAP-02V-S / SPHD-002T-P0.5"
        branch_rows.append({"bus_id": bus_id, "protocol": bus["protocol"], "axis_count": len(axis_ids), "axis_ids": " | ".join(axis_ids), "power_corridor_chain": pwr, "data_corridor_chain": data, "candidate_12v_stall_endpoint_sum_a": f"{endpoint:.2f}", "endpoint_use_boundary": "MOMENTARY DATASHEET STALL SUM ONLY - NOT NORMAL DEMAND OR A PROTECTION/CONDUCTOR RATING", "power_topology": "ONE SEPARATELY PROTECTED SEGMENT BRANCH; LISTED AXES MAY SHARE ONLY THIS BRANCH VDD", "data_power_boundary": "DATA-ONLY FIELD CONNECTOR HAS NO ACTUATOR VDD CONTACT; FINAL POWER-INJECTION BREAKOUT AND NO-BACKFEED TEST REMAIN OPEN", "controller_interface": f"Carrier {pinout['carrier']}; {pinout['interface_device']}; {pinout['stm32_peripheral']}; {field_header}", "termination_bias_level_shift": "INTERFACE DEVICE SELECTED; PASSIVES, TERMINATION/BIAS, PROTECTION AND PCB VALIDATION REQUIRED", "status": "PIN-LEVEL ROUTED CANDIDATE - NOT RELEASED"})
        boundaries.append({"boundary_id": f"CB-{bus_id}", "boundary_type": "CONTROLLER / SEGMENT DATA-ONLY", "bus_id": bus_id, "axis_id": "N/A", "known_side": f"Carrier {pinout['carrier']} {pinout['interface_device']} / {pinout['stm32_peripheral']}", "known_pin_mapping": field_header, "mating_part": f"JST {field_housing}; ASSEMBLED CABLE/CRIMP APPLICATION SELECTION REQUIRED", "validation": "SOURCE-VERIFIED CONDUCTOR-RANGE CANDIDATE; RECEIVED INSULATION OD/PCB/CABLE/EMC/FAULT TEST NOT EXECUTED"})
    ...
    pinned_prefixes = ("AX_", "MCU_IO_", "JMCU_", "JCA", "JCB", "J_RS", "J_TTL", "ISO_", "LVL_")
    terminal_binding = [{"sheet": row["sheet"], "reference": row["reference"], "terminal": row["terminal"], "pin_name": row["pin_name"], "net": row["net"], "current_definition_state": row["status"], "physical_connector_instance": "SOURCE-VERIFIED DEVICE/CONNECTOR CANDIDATE" if row["reference"].startswith(pinned_prefixes) else "SELECTION REQUIRED", "physical_contact": row["terminal"] if row["reference"].startswith(pinned_prefixes) else "SELECTION REQUIRED", "wire_or_cable_core": "SELECTION REQUIRED", "route_segment": "BINDING REQUIRED", "validation": "PIN IDENTITY RECORDED; PCB/HARNESS/PHYSICAL TEST NOT EXECUTED" if row["reference"].startswith(pinned_prefixes) else "NOT EXECUTED"} for row in terminal_rows]
    ...
    source_rows += [
        {"source_id": "STM32H743ZIT6", "official_url": ST_H743_SOURCE, "revision_or_date": "ST DS12110 REV 11", "accessed_date": ACCESSED, "verified": "LQFP144 package pins and UART alternate functions used by all eight segments", "not_verified": "carrier PCB implementation, timing and physical validation"},
        {"source_id": "ISOW1432DFMR", "official_url": TI_ISOW1432_SOURCE, "revision_or_date": "TI SLLSF86C REV C; 2022-03", "accessed_date": ACCESSED, "verified": "20-pin DFM package pinout and active orderable device candidate", "not_verified": "HR-30 decoupling, layout, termination, protection and EMC"},
        {"source_id": "SN74LVC1T45DCKR", "official_url": TI_LVC1T45_SOURCE, "revision_or_date": "TI SCES515N REV N; 2024-06", "accessed_date": ACCESSED, "verified": "DCK/SC70-6 package pinout and active orderable device candidate", "not_verified": "HR-30 sequencing, protection and single-wire waveform behavior"},
        {"source_id": "JST-GH", "official_url": JST_GH_SOURCE, "revision_or_date": "CURRENT OFFICIAL CATALOG; REVISION DATE NOT STATED", "accessed_date": ACCESSED, "verified": "GHR-15V-S/BM15B-GHS-TBT/SSHL-002T-P0.2 logic-only controller cable family", "not_verified": "assembled cable, flex life, retention, environmental and EMC performance"},
        {"source_id": "JST-PA", "official_url": JST_PA_SOURCE, "revision_or_date": "CURRENT OFFICIAL CATALOG; REVISION DATE NOT STATED", "accessed_date": "2026-08-18", "verified": "PAP-02V-S/PAP-03V-S housings; B02B-PASK-1/B03B-PASK-1 through-hole secure-lock headers; SPHD-001T-P0.5 0.13-0.33 mm2 and SPHD-002T-P0.5 0.08-0.21 mm2 contacts", "not_verified": "received conductor insulation O.D., crimp tooling, pull strength, flex life, retention, environmental and EMC performance"},
    ...
    status = {"identifier": "HR30-WHOLE-BODY-HARNESS-P0.1", "axis_drop_count": 25, "bus_branch_count": 8, "corridor_count": 12, "connector_boundary_count": 33, "harness_assembly_count": len(assembly_rows), "installed_equipment_binding_count": len(equipment_rows), "service_loop_obligation_count": len(loop_rows), "logical_terminal_binding_count": len(terminal_binding), "actuator_side_pinout_families_verified": 4, "candidate_12v_stall_endpoint_sum_a": total_endpoint, "u2d2_final_controller_role": "REJECT", "u2d2_power_hub_whole_body_role": "REJECT", "actuator_bus_controller_pin_map_selected": True, "controller_side_data_connector_candidates_selected": True, "interface_device_candidates_selected": True, "controller_side_connectors_selected": False, "assembled_cables_selected": False, "protection_selected": False, "conductor_sizing_released": False, "harness_validated": False, "connection_authority": False, "fabrication_authority": False, "powered_test_authority": False, "motion_authority": False, "energization_authority": False, "warning": WARNING}
