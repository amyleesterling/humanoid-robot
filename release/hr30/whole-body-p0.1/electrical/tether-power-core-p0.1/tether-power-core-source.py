...
def write_docs(sheets):
    ...
    web = web_path.read_text(encoding="utf-8").replace("2</b>series GV121CAC candidates", "2</b>LC1D40ABD candidates")
    web = web.replace('<div class="hold"><h2>Fuse values are intentionally absent</h2>', '<div class="hold"><h2>The EDM hardware mismatch is corrected</h2><p>K1 and K2 each use explicit 21-22 mirror-certified NC auxiliary contacts. Their three main poles are wired in series. This is still an unvalidated candidate circuit: no achieved Category, PL, SIL or permission to energize is claimed.</p></div><div class="hold"><h2>Fuse values are intentionally absent</h2>')
