...
def build() -> None:
    ...
    write_csv(OUT / "assembly-traveler.csv", [
        ...
        {"step":"A04","operation":"make and destructively test one process coupon before production contacts","acceptance":"18 AWG conductor crimp height 1.00-1.10 mm and pull force >88.0 N; insulation crimp removed from pull influence","record":"BH-R06","state":"NOT EXECUTED","stop_rule":"STOP until process passes"},
    ...
    write_csv(OUT / "as-built-record.csv", [
        ...
        {"record_id":"BH-R06","field":"destructive crimp process coupon","required_value":"1.00-1.10 mm and >88.0 N","observed_value":"NOT RECORDED","operator":"UNASSIGNED","reviewer":"UNASSIGNED","date_utc":"NOT EXECUTED","result":"OPEN"},
    ...
    write_csv(OUT / "open-holds.csv", [
        ...
        {"hold_id":"BH-H02","unresolved_evidence":"received exact Mueller leads, Molex connectors/tooling, lot records and calibration certificates","state":"OPEN","authority":AUTHORITY},
...
def integrate() -> None:
    ...
    status.update({
        ...
        "bench_harness_exact_mini_fit_tool_selected":True,"bench_harness_manufacturer_assembled_source_end_selected":True,
    ...
    for row in holds:
        if row["hold_id"] == "CS-H04":
            row["unresolved_evidence"] = "received Mueller lead identity/length/jacket OD/conductor construction, complete tin-dipped-segment removal, received 63819-0901/11-03-0044 tooling and calibration, executed 18 AWG Mini-Fit crimp-height/pull/polarity records and signed BH-A01 as-built disposition"
    write_csv(holds_path, holds)
    start, end = "<!-- BENCH-HARNESS-P01 START -->", "<!-- BENCH-HARNESS-P01 END -->"
    block = f'''{start}<h2>Assembly-controlled bench harness</h2><div class="panel"><p>The two-wire source lead now uses exact manufacturer-assembled Mueller 39-inch source leads, an exact Mini-Fit contact map, tin-dipped-tip removal, strip dimensions, candidate 63819-0901 crimp tool, measurable 18 AWG crimp and pull criteria, a ten-step traveler and a thirteen-field as-built record. Received lead compatibility and all physical evidence remain open.</p><p><a href="bench-harness-p0.1/index.html">Open the interactive bench-harness guide</a> · <a href="bench-harness-p0.1/bench-harness.svg">assembly drawing</a> · <a href="bench-harness-p0.1/as-built-record.csv">as-built record</a>.</p></div>{end}'''
    ...
    station_readme.write_text(text + f"\n\n{start}\n## Bench harness\n\nThe current-limited source lead is defined in `bench-harness-p0.1/` with exact manufacturer-assembled Mueller source leads, explicit contacts, sacrificial tin-dipped-tip removal, wire preparation, tooling, inspection traveler and as-built record. Received lead compatibility, fabrication, inspection, qualified review and all connection/power authority remain open.\n{end}\n", encoding="utf-8")
    ...
    root_status.update({
        ...
        "axis_commissioning_bench_harness_contact_map_complete":True,"axis_commissioning_bench_harness_manufacturer_assembled_source_end_selected":True,
    ...
    root_block = f'''{start}<section id="bench-harness"><h2>Commissioning bench harness</h2><div class="grid"><article class="card pass"><h3>Exact manufacturer source leads</h3><p>Red and black Mueller 39-inch shrouded-banana leads remove the custom plug-termination ambiguity.</p></article><article class="card pass"><h3>Explicit J1 polarity</h3><p>Black/GND is cavity 1; red/VDD is cavity 2. Tip removal, strip and 18 AWG crimp criteria are controlled.</p></article><article class="card hold"><h3>Physical evidence open</h3><p>Received lead compatibility, tooling, coupon, inspection and connection authority remain unresolved.</p></article></div><p><a href="electrical/axis-commissioning-station-p0.1/bench-harness-p0.1/index.html">Open the interactive bench-harness guide</a>.</p></section>{end}'''
