...
SOURCES = [
    ...
    ("SRC-08", "Sensata GIGAVAC", "GV12 series", "https://www.sensata.com/sites/default/files/a/sensata-gigavac-gv12-series-100v-contactors-datasheet.pdf", "datasheet rev 2022-04-28; accessed 2026-08-14", "100 V / 200 A family, 12 V coil, optional mechanically linked auxiliary; exact EDM suitability requires qualified confirmation"),
...
def build() -> dict:
    ...
    energy_terms = [r for r in terminals if r["sheet"].startswith(("01_", "02_"))]
    ...
    tree = [
        ...
        ("PWR-04","robot tether inlet","robot PDU","ACT_0V_CONTROLLED","five RS-485 actuator branches","12 V main candidate; each branch separately protected, values open"),
        ("PWR-05","robot tether inlet","three S18V20F9 candidates","ACT_0V_CONTROLLED","three 9 V TTL actuator branches","one 2 A-typical converter per 1.76 A stall-endpoint branch; thermal/transient proof open"),
    ...
    devices = [
        ...
        ("DEV-04","series interruption K1/K2","two GV12-family contactors","EVALUATION CANDIDATE","external only","0.44 kg and 8 W coil each; auxiliary is mechanically linked but not claimed as an IEC mirror contact"),
    ...
    safety = [
        ...
        ("SF-03","external-device monitoring","one feedback contact from each K1/K2 in reset loop","block reset after failed opening","no automatic restart","exact auxiliary-contact semantics and common-cause analysis open"),
    ...
    mass = [
        ...
        ("M-03","keep two GV12 contactors external",0.000,"avoids 0.88 kg onboard penalty"),
...
def write_visuals(stats: dict) -> None:
    svg = f'''<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1400 720" role="img" aria-labelledby="t d"><title id="t">HR-30 tether-first energy and safety spine</title><desc id="d">External mains enclosure containing supply, safety relay and two series contactors feeding the robot power distribution and regulated rails.</desc><style>text{{font-family:Arial,sans-serif;fill:#0d2d57}}.box{{fill:#fff;stroke:#123f73;stroke-width:3}}.safe{{fill:#d8f1ff}}.hold{{fill:#fff2bf}}.line{{stroke:#123f73;stroke-width:5;fill:none;marker-end:url(#a)}}.dash{{stroke:#d79800;stroke-dasharray:10 8}}.h{{font-size:22px;font-weight:700}}.b{{font-size:16px}}.s{{font-size:14px}}</style><defs><marker id="a" markerWidth="10" markerHeight="10" refX="8" refY="3" orient="auto"><path d="M0 0L9 3L0 6z" fill="#123f73"/></marker></defs><rect width="1400" height="720" rx="28" fill="#f7fcff"/><text x="42" y="50" class="h">Tether-first P0.1: high-energy and mains equipment stays in the external panel</text><rect class="box hold" x="40" y="90" width="515" height="550" rx="22"/><text x="68" y="130" class="h">Qualified external enclosure</text><rect class="box" x="75" y="170" width="190" height="105" rx="16"/><text x="95" y="205" class="h">RSP-500-12</text><text x="95" y="235" class="b">12 V / 41.7 A candidate</text><text x="95" y="260" class="s">Mains/PE/protection open</text><rect class="box safe" x="315" y="170" width="205" height="105" rx="16"/><text x="335" y="205" class="h">PNOZ s4</text><text x="335" y="235" class="b">E-stop + manual reset</text><text x="335" y="260" class="s">EDM / validation open</text><rect class="box" x="75" y="340" width="180" height="100" rx="16"/><text x="100" y="378" class="h">K1</text><text x="100" y="410" class="b">DC contactor</text><rect class="box" x="325" y="340" width="180" height="100" rx="16"/><text x="350" y="378" class="h">K2</text><text x="350" y="410" class="b">DC contactor</text><path class="line" d="M265 222H305"/><path class="line dash" d="M420 275V330"/><path class="line" d="M165 275V330"/><path class="line" d="M255 390H315"/><rect class="box safe" x="75" y="500" width="430" height="100" rx="16"/><text x="98" y="535" class="h">Touch-safe tether boundary</text><text x="98" y="567" class="b">strain relief / breakaway / rating: SELECTION REQUIRED</text><path class="line" d="M415 440V490"/><rect class="box safe" x="660" y="90" width="700" height="550" rx="22"/><text x="690" y="130" class="h">HR-30 robot</text><rect class="box" x="705" y="180" width="220" height="105" rx="16"/><text x="730" y="216" class="h">Robot PDU</text><text x="730" y="246" class="b">8 protected branches</text><text x="730" y="270" class="s">all values unresolved</text><path class="line" d="M505 550H650V232H695"/><rect class="box" x="1010" y="155" width="290" height="105" rx="16"/><text x="1035" y="193" class="h">5 XH/XM branches</text><text x="1035" y="225" class="b">controlled main rail</text><path class="line" d="M925 220H1000"/><rect class="box" x="1010" y="310" width="290" height="115" rx="16"/><text x="1035" y="348" class="h">3 XC330 branches</text><text x="1035" y="380" class="b">one 9 V regulator each</text><text x="1035" y="406" class="s">2 A typical; proof open</text><path class="line" d="M925 235H965V367H1000"/><rect class="box hold" x="705" y="485" width="595" height="110" rx="16"/><text x="730" y="522" class="h">Onboard later: BLF-1209WS LiFePO4 candidate</text><text x="730" y="554" class="b">18 A continuous / 40 A for 2 s &lt; 60.58 A provisional peak</text><text x="730" y="580" class="s">cassette, interruption, charge and fault architecture remain open</text></svg>'''
    (OUT / "energy-safety-spine.svg").write_text(svg,encoding="utf-8")
    page = f'''<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>HR-30 energy and safety spine</title><style>:root{{--navy:#0d2d57;--blue:#179de3;--sky:#d8f1ff;--gold:#f4b400;--paper:#f7fcff}}*{{box-sizing:border-box}}body{{margin:0;background:var(--paper);color:var(--navy);font:16px/1.5 system-ui,sans-serif}}header{{padding:30px max(20px,calc((100% - 1180px)/2));color:white;background:linear-gradient(135deg,var(--navy),#1769aa)}}main{{max-width:1180px;margin:auto;padding:28px 20px 70px}}h1{{font-size:clamp(34px,5vw,62px);line-height:1.05}}h2{{font-size:clamp(25px,3vw,38px);margin-top:44px}}.warning{{padding:14px 18px;border-radius:12px;background:var(--gold);color:#19273d;font-weight:800}}.grid{{display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:14px}}.card,.panel{{background:#fff;border:2px solid #9bd5f5;border-radius:16px;padding:18px;box-shadow:0 6px 18px #0d2d5712}}.card strong{{display:block;font-size:30px}}.hold{{border-left:8px solid var(--gold)}}img{{display:block;width:100%;height:auto}}a{{color:#075f9f;font-weight:700}}li{{margin:.7em 0}}@media(max-width:600px){{header{{padding:24px 18px}}main{{padding:20px 14px}}}}</style></head><body><header><div class="warning">{html.escape(WARNING)}</div><p>HR-30 whole humanoid · P0.1</p><h1>Energy &amp; safety spine</h1><p>A whole-robot power architecture that starts tethered, keeps mains and heavy safety equipment off the body, and separates reset from motion.</p></header><main><div class="grid"><div class="card"><strong>{stats['operating_budget_w']:.0f} W</strong>provisional operating budget</div><div class="card hold"><strong>{stats['short_peak_budget_w']:.0f} W</strong>unclosed short-peak estimate</div><div class="card"><strong>{stats['tether_supply_limit_a']:.1f} A</strong>tether candidate output limit</div><div class="card hold"><strong>0</strong>released fuse values or conductor sizes</div></div><h2>Tether-first topology</h2><div class="panel"><img src="energy-safety-spine.svg" alt="Tether-first HR-30 energy and safety architecture diagram"></div><h2>What changed</h2><div class="grid"><div class="card"><h3>Direct 4S LiPo rejected</h3><p>Its 14.8 V nominal voltage equals the XH/XM published maximum. The design no longer treats it as a direct actuator source.</p></div><div class="card"><h3>TTL rails move to 9 V</h3><p>Each two-axis XC330 branch gets its own regulator candidate instead of operating at the 12.0 V maximum.</p></div><div class="card"><h3>Reset is not run</h3><p>Manual reset can only restore safety-output eligibility. A separate deterministic motion command is always required.</p></div><div class="card hold"><h3>Peak remains open</h3><p>727 W equals 60.58 A at 12 V. Neither source candidate closes it, so torque/current limits are mandatory development inputs.</p></div></div><h2>Two configurations, one boundary</h2><div class="panel"><ul><li><b>Tether-first:</b> external enclosed supply, safety relay and two series contactors; robot-side PDU and regulated branch rails.</li><li><b>Onboard later:</b> 12 V 9 Ah LiFePO4 evaluation candidate only after tether load data, cassette redesign, fault current, charge and interruption evidence close.</li></ul></div><h2>Controlled registers</h2><div class="panel"><p><a href="configuration-register.csv">configurations</a> · <a href="power-tree-register.csv">power tree</a> · <a href="candidate-device-register.csv">devices</a> · <a href="voltage-compatibility-register.csv">voltage checks</a> · <a href="current-power-budget.csv">current/power budget</a> · <a href="safety-function-boundary.csv">safety boundary</a> · <a href="fault-response-register.csv">fault responses</a> · <a href="terminal-interface-register.csv">current ECAD terminals</a> · <a href="first-energization-sequence.csv">staged test gates</a> · <a href="unresolved-input-register.csv">open inputs</a> · <a href="source-register.csv">primary sources</a></p></div><h2>No energization claim</h2><div class="panel hold"><p>The native KiCad energy sheets still need a physical pin-level correction pass. Protection values, conductor sizes, contactor order codes, PE/0 V topology, stopping time, PLr/SIL allocation, regeneration, precharge and physical validation remain open. This package is design architecture—not approval.</p></div></main></body></html>'''
...
def manifest_release_integrate(stats: dict) -> None:
    ...
    if marker not in text: rp.write_text(text+block,encoding="utf-8")
