...
- [HR-V0 integrated arm architecture P0.5](docs/hr-v0-arm-architecture-p0.5.md)
- [HR-V0 integrated mechanical release candidate P0.4](docs/hr-v0-mechanical-release-p0.4.md)
...
- [Native KiCad Electrical V3-P1.8 correction candidate](electrical/kicad/project-button-v3/README.md)
...
Generate exact vendor-coordinate evidence with `C:\Users\amyle\Documents\New project\.venvs\hr-v0-cad\Scripts\python.exe tools/generate_hr_v0_robotis_interface.py`, then run its checker. Generate the current R66 / `HR-V0-ARM-ARCH-P0.5` candidate with the same interpreter using `tools/generate_hr_v0_arm_architecture.py`, then run `tools/check_hr_v0_arm_architecture.py`. Run `tools/generate_hr_v0_mechanical_release.py` and its checker after deliberate mechanical-control changes. `HR-V0-MECH-P0.4` remains the release hold and all arm fabrication packets remain withdrawn until received fit/MTR/FAI, T-slot proof, qualified analytical acceptance, fastener installation controls, cables/guard, continuous collision proof, hard-stop/overtravel margin, physical proof and qualified review close `MECH-005` and `MECH-006`.
...
Sixty-six review/control rounds are complete: R01-R66. R11 Fable and R12 Sol are independent parallel reviews of the same pre-correction baseline. The resupplied Sol verdict is the same R12 analysis and is not double-counted. R53-R66 are project-owned exact-geometry, release-boundary, sourcing, physical-panel, H1, protection, service-disconnect, transport, and mechanical-integration corrections, not additional independent reviews.
