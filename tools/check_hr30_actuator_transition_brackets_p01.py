...
def main() -> int:
    ...
    coupon_matrix = rows(OUT / "coupon-opening-matrix.csv")
    coupon_process = rows(OUT / "coupon-process-traveler.csv")
    coupon_inspection = rows(OUT / "coupon-inspection-register.csv")
    ...
    need(len(parts) == 3 and len(interfaces) == 5 and len(holds) == 6 and len(sources) == 9, "controlled register count drift")
    need(len(coupon_matrix) == len(coupon_inspection) == 9 and len(coupon_process) == 8, "diagnostic coupon register count drift")
    need({r["nominal_width_mm"] for r in coupon_matrix} == {"13.500","14.000","14.500"}, "coupon width matrix drift")
    need({r["nominal_height_mm"] for r in coupon_matrix} == {"9.000","9.500","10.000"}, "coupon height matrix drift")
    need(all(r["execution_state"] == "NOT EXECUTED" for r in coupon_process), "coupon process falsely executed")
    need(all(r["disposition"] == "OPEN - NO FIT CLAIM" for r in coupon_inspection), "coupon inspection falsely completed")
    ...
    required = ["ATB-BASE-P0.1.step","ATB-CLAMP-CAP-P0.1.step","ATB-CONNECTOR-FIT-COUPON-P0.1.step","ATB-CONNECTOR-FIT-COUPON-P0.1.glb","ATB-STANDARD-ASSEMBLY-P0.1.step","ATB-STANDARD-ASSEMBLY-P0.1.glb","HR30_25_axis_transition_brackets_candidate.step","HR30_25_axis_transition_brackets_candidate.glb"]
    ...
    need(status["diagnostic_fit_coupon_present"] is True and status["diagnostic_fit_coupon_opening_count"] == 9, "coupon status drift")
    need(status["diagnostic_fit_coupon_material_candidate_defined"] is True and status["diagnostic_fit_coupon_process_traveler_present"] is True, "coupon preparation status drift")
    need(status["diagnostic_fit_coupon_physical_test_executed"] is False, "coupon physical test falsely executed")
    ...
    need("font:17px" in page and "font-size:14px" in page and "project-owned clearance proxy" in page.lower() and "Print the connector-fit coupon first" in page, "guide content/legibility drift")
    ...
    need(root_status["actuator_transition_bracket_diagnostic_fit_coupon_present"] is True and root_status["actuator_transition_bracket_coupon_opening_count"] == 9 and root_status["actuator_transition_bracket_coupon_physical_test_executed"] is False, "root coupon integration drift")
    need("HR30-TRANSITION-BRACKETS-P01-START" in (WB / "index.html").read_text(encoding="utf-8"), "root web integration missing")
    print("PASS: 25 dimensioned transition-cassette placements plus a nine-window diagnostic fit coupon are synchronized; connector fit, production fabrication and all power/motion authority remain open")
    return 0
