#!/usr/bin/env python3
"""Generate the fail-closed watchdog PCB/BOM binding synchronized through R195."""
...
def main() -> None:
    ...
    status = {
        ...
        "date": "2026-08-10",
        "round": "R149+R150+R195-SYNCHRONIZED",
        "bom_item_id": "BOM-048",
        "board": "PCB-P1.0 / Electrical V3-P1.15-CARRIER-CANDIDATE (DIRECT NATIVE BINDING)",
        "native_board_title_revision": "PCB-P1.0 / Electrical V3-P1.15",
    ...
    page = page.replace("synchronized R166", "synchronized R195")
    page = page.replace("PCB-P0.9", "PCB-P1.0")
    page = page.replace("R166 adds P1.15-bound source and parity hashes", "R195 directly binds the native board title to P1.15 and refreshes source hashes")
    page = page.replace(", bound to <code>HR-V0-E2-P115-PARITY-P0.1</code>", ", directly bound to Electrical V3-P1.15")
    (OUT / "index.html").write_text(page, encoding="utf-8")

    print(f"{IDENTIFIER}: PCB-P1.0 / 42 populated references / 16 BOM lines / 4 NPTH features")
