...
def main() -> None:
    ...
    row = {
        ...
        "release_state": "EXACT CANDIDATE HOLD - CURRENT CAM AND ASSEMBLY PROCESS ABSENT",
        "warning": WARNING,
    }
    ...
    facts = [
        ...
        ("0", "current CAM or machine-ready XYRS files"),
    ]
    cards = "".join(f'<article class="card"><div class="metric">{value}</div>{html.escape(label)}</article>' for value, label in facts)
    page = f'''<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>HR-V0 watchdog PCB BOM binding</title><style>:root{{--sky:#b9e8ff;--navy:#082f58;--blue:#12669f;--gold:#f2b928;--paper:#f5fbff;--hold:#fff0b8}}*{{box-sizing:border-box}}body{{margin:0;color:var(--navy);font:clamp(16px,1.1vw,19px)/1.5 Arial,sans-serif}}header{{padding:clamp(1.5rem,5vw,4rem);background:linear-gradient(135deg,var(--sky),#effaff);border-bottom:7px solid var(--gold)}}h1{{font-size:clamp(2.3rem,5.5vw,4.8rem);line-height:1.03;max-width:18ch;margin:.25rem 0 1rem}}h2{{font-size:clamp(1.6rem,3vw,2.7rem)}}main{{max-width:1380px;margin:auto;padding:2rem clamp(1rem,4vw,3.5rem)}}.warning{{background:var(--hold);border:3px solid #ad7500;border-radius:.8rem;padding:1rem;font-weight:800}}.grid{{display:grid;grid-template-columns:repeat(auto-fit,minmax(min(100%,230px),1fr));gap:1rem;margin:2rem 0}}.card{{border:3px solid var(--blue);border-radius:1rem;padding:1.1rem;background:var(--paper)}}.metric{{font-size:clamp(2rem,4vw,3.5rem);font-weight:900}}.boundary{{border-left:7px solid var(--gold);padding-left:1rem;margin:2rem 0}}.table-wrap{{overflow:auto;border:2px solid #93b8ce;border-radius:.7rem}}table{{border-collapse:collapse;width:100%;min-width:950px}}th,td{{padding:.8rem;text-align:left;vertical-align:top;border-bottom:1px solid #bdd0dc}}th{{background:var(--navy);color:white}}code{{font-size:14px;overflow-wrap:anywhere}}a{{color:#075d98}}</style></head><body><header><div>{IDENTIFIER} · R149 · 2026-08-09</div><h1>The live BOM now points to watchdog board PCB-P0.9.</h1><div class="warning">{WARNING}. Historical PCB-P0.5 CAM evidence remains quarantined and must not be uploaded, ordered, or fabricated.</div></header><main><p>BOM-048 previously named PCB-P0.5. It now binds the current native PCB-P0.9 source to the P0.2 assembly BOM and placement reference by SHA-256.</p><section class="grid">{cards}</section><div class="boundary"><h2>What the exact identity proves</h2><p>The current configuration contains one native board, 42 populated references represented by 16 exact-MPN BOM lines, 42 internal placement-reference rows, and four unpopulated mounting holes. Native DRC is checked elsewhere. This package proves only BOM/source agreement.</p></div><h2>Controlled files</h2><div class="table-wrap"><table><thead><tr><th>Role</th><th>Repository path</th><th>SHA-256</th><th>Supplier use</th></tr></thead><tbody><tr><td>Native PCB</td><td><code>{html.escape(row["native_pcb_path"])}</code></td><td><code>{row["native_pcb_sha256"][:20]}…</code></td><td>INTERNAL ONLY</td></tr><tr><td>Assembly BOM</td><td><code>{html.escape(row["assembly_bom_path"])}</code></td><td><code>{row["assembly_bom_sha256"][:20]}…</code></td><td>NOT RELEASED</td></tr><tr><td>Placement reference</td><td><code>{html.escape(row["placement_path"])}</code></td><td><code>{row["placement_sha256"][:20]}…</code></td><td>NOT MACHINE XYRS</td></tr></tbody></table></div><div class="boundary"><h2>Twelve holds remain open</h2><p>Supplier coordinates, reference-level DFM, sourcing/traceability, SMT and THT processes, insulation/cleanliness, Pico process, fabrication definition, first article, independent qualified review, immutable supplier packet, and separate work authorization remain open.</p></div><p><a href="../../../bom/hr-v0-watchdog-pcb-binding.csv">Download the binding</a> · <a href="../watchdog-pcba-assembly-data-p0.2/index.html">Open the assembly-data guide</a></p></main></body></html>'''
    (OUT / "index.html").write_text(page, encoding="utf-8")

    print(f"{IDENTIFIER}: PCB-P0.9 / 42 populated references / 16 BOM lines / 4 NPTH features")
    print("BOM-048 exact candidate only; current CAM, supplier XYRS, fabrication, assembly and energization remain false")
    print(WARNING)
