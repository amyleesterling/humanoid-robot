...
def main() -> int:
    ...
    status=json.loads((SRC/"pdu-status.json").read_text(encoding="utf-8")); need(status["native_schematic_sheet_count"]==8 and status["allocated_axis_channels"]==25 and status["dnp_spare_channels"]==5,"PDU status count drift"); need(status["placement_complete"] and status["routing_complete"] and status["drc_accepted"],"routed candidate status drift"); need(status["copper_layer_count"]==10 and status["board_width_mm"]==124.0 and status["board_height_mm"]==45.0 and not status["production_stackup_selected"],"compact board geometry/stackup boundary drift"); need(not any(status[k] for k in ("walking_power_architecture_complete","reverse_energy_architecture_complete","connector_current_compatibility_validated","thermal_validated","functional_safety_credit","procurement_authority","fabrication_authority","connection_authority","powered_test_authority","motion_authority","energization_authority")),"PDU authority overclaim")
    ...
    page=(SRC/"index.html").read_text(encoding="utf-8"); need("not the walking power stage" in page.lower() and "drc 0/0" in page.lower() and "124 × 45 mm" in page and "production stackup" in page.lower(),"interactive guide hides compact routed/preliminary boundary")
