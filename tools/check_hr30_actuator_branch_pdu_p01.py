...
def main() -> int:
    required={"hr30-actuator-branch-pdu-p0.1.kicad_pro","hr30-actuator-branch-pdu-p0.1.kicad_sch","hr30-actuator-branch-pdu-p0.1.kicad_pcb","01_pdu_boundaries.kicad_sch",*[f"{i+1:02d}_channel_{i}.kicad_sch" for i in range(1,7)],"board-instance-channel-allocation.csv","current-limit-torque-consequence-register.csv","component-register.csv","terminal-register.csv","primary-source-register.csv","open-holds.csv","pdu-status.json","index.html","README.md","validation/hr30-actuator-branch-pdu-p0.1-erc.rpt","validation/hr30-actuator-branch-pdu-p0.1-drc.rpt","file-manifest.csv"}
    ...
    status=json.loads((SRC/"pdu-status.json").read_text(encoding="utf-8")); need(status["native_schematic_sheet_count"]==8 and status["allocated_axis_channels"]==25 and status["dnp_spare_channels"]==5,"PDU status count drift"); need(status["placement_complete"] and not status["routing_complete"] and not status["drc_accepted"],"placed/unrouted boundary drift"); need(not any(status[k] for k in ("walking_power_architecture_complete","reverse_energy_architecture_complete","connector_current_compatibility_validated","thermal_validated","functional_safety_credit","procurement_authority","fabrication_authority","connection_authority","powered_test_authority","motion_authority","energization_authority")),"PDU authority overclaim")
    ...
    drc=(SRC/"validation/hr30-actuator-branch-pdu-p0.1-drc.rpt").read_text(encoding="utf-8"); need("unconnected" in drc.lower(),"fail-closed unrouted DRC evidence missing")
    holds=rows("open-holds.csv"); need(len(holds)>=8 and all(r["state"].startswith("OPEN") for r in holds),"PDU holds not fail-closed")
    page=(SRC/"index.html").read_text(encoding="utf-8"); need("not the walking power stage" in page.lower() and "ratsnest is intentional" in page.lower(),"interactive guide hides preliminary boundary")
    print("PASS: HR-30 PDU has 25 allocated branch slots; schematic ERC 0/0; PCB routing/DRC and every work authority remain open")
    return 0
