...
def main() -> None:
    ...
    for product in release["current_products"]:
        if product.get("domain") in {"electrical","mechanical","bill_of_materials","commissioning","assembly"}: need(product.get("configuration_reconciliation") in {gen.CID,"HR-V0-CONFIG-REC-P0.33","HR-V0-CONFIG-REC-P0.34"},f"release config {product.get('domain')}")
    ...
    need((ROOT/"docs/handoff-current.md").read_text(encoding="utf-8").startswith(("R268 functional datum/GD&T correction:","R269 J2 hard-stop strength correction:","R270 corrected J2 contact/load model:")),"handoff")
