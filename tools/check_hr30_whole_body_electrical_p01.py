...
def main() -> int:
    ...
    require(status["native_kicad_parsed"] and status["logical_connectivity_reconciled"] and status["actuator_side_physical_pin_mapping_reconciled"] and status["actuator_bus_controller_physical_pin_mapping_reconciled"] and status["actuator_bus_interface_device_candidates_selected"] and status["actuator_bus_data_only_connector_candidates_selected"] and status["tether_first_energy_topology_encoded"] and status["direct_14v8_actuator_source_absent"] and status["individual_actuator_power_feed_count"] == 25 and status["regulated_ttl_branch_count"] == 3 and status["three_rail_auxiliary_architecture_encoded"] and status["auxiliary_converter_candidate"] == "3x RECOM REC30E-2405SZ" and status["auxiliary_positive_rails"] == ["COMPUTE_5V1", "HMI_5V0", "AUX_5V_SAFE"] and status["auxiliary_return_star"] == "AUX_0V_STAR" and status["auxiliary_hmi_zero_peak_headroom_blocker"] and status["reset_can_command_motion"] is False and status["erc_errors"] == status["erc_warnings"] == 0, "native electrical status incomplete")
