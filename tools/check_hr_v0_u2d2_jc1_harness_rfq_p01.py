...
def main() -> int:
    ...
    for domain in ("electrical", "bill_of_materials", "assembly"):
        ...
        need(product.get("configuration_reconciliation") in {CID, "HR-V0-CONFIG-REC-P0.27", "HR-V0-CONFIG-REC-P0.28"} and product.get("u2d2_jc1_harness_rfq") == ID and ID in product.get("supporting_identifiers", []) and CID in product.get("supporting_identifiers", []), f"release metadata stale: {domain}")
