...
ROOT=Path(__file__).resolve().parents[1];OUT=ROOT/"mechanical/analysis/hr-v0-j2-c07-pe-seam-free-jacobian-mesh-p0.1";RELEASE=ROOT/"release/hr-v0/j2-c07-pe-seam-free-jacobian-mesh-p0.1";GEN=ROOT/"tools/generate_hr_v0_j2_c07_pe_seam_free_jacobian_mesh_p01.py";PRIOR=ROOT/"tools/generate_hr_v0_j2_c07_pe_seam_free_mesh_p01.py";BASE=ROOT/"tools/generate_hr_v0_j2_c07_conformal_zone_mesh_p01.py";PREREG=ROOT/"mechanical/analysis/hr-v0-j2-c07-pe-seam-free-jacobian-prereg-p0.1/frozen-jacobian-successor-protocol.json";R298=ROOT/"mechanical/analysis/hr-v0-j2-c07-pe-seam-free-mesh-p0.1/analysis-status.json";R298_PROVENANCE=R298.parent/"execution-provenance.json"
...
def main()->int:
    ...
    r298_provenance=json.loads(R298_PROVENANCE.read_text(encoding="utf-8"))
    if st["candidate_id"]!=protocol["candidate_id"] or st["preregistration_sha256"]!=sha(PREREG) or st["r298_baseline_status_sha256"]!=r298_provenance["pre_raw_shard_migration_status_sha256"]:fail("input binding")
    if prov["generator_sha256"]!=sha(GEN) or prov["transitive_r298_generator_sha256"]!=sha(PRIOR) or prov["transitive_r289_generator_sha256"]!=sha(BASE):fail("generator binding")
    if len(rows("bore-wall-field-resolution.csv"))!=4 or any(r["gate"]!="PASS" for r in rows("bore-wall-field-resolution.csv")):fail("face resolution")
    ...
    if len(summaries)!=21:fail("zone count")
    for s in summaries:
    ...
    monitored=not any(r["zone_id"]!="C07-MATRIX" and r["monitored_min_0p20_gate"]!="PASS" for r in summaries);fused=[r for r in summaries if r["zone_id"]=="C07-PE-FUSED"][0];fused_gate=float(fused["minimum_sicn"])>=.20
    ...
    if st["global_sicn_gate"]!=global_gate or st["monitored_zone_minimum_gate"]!=monitored or st["actual_quadrature_signed_jacobian_gate"]!=jac_gate or st["r279_c02_complete"]!=c02 or st["fused_pe_quality_gate"]!=fused_gate:fail("gate derivation")
    ...
    print(f"PASS: R300 synchronized; tets={len(q)} global={global_gate} monitored={monitored} fused/subzones={fused_gate} jacobian={jac_gate} R279-C02={c02}; structural/H02/capacity/all authority open");return 0
