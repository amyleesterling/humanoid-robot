...
def main() -> int:
    ...
    need((ROOT / "docs/handoff-current.md").read_text(encoding="utf-8").startswith(("R277 J2 pad-pocket correction:", "R278 exact-normal J2 stop correction:")), "handoff drift")
    need((ROOT / "docs/review-ledger.md").read_text(encoding="utf-8").count("| R277 |") == 1, "ledger missing or duplicated")
    need(any(text in (ROOT / "README.md").read_text(encoding="utf-8") for text in ("Two hundred seventy-seven rounds are complete", "Two hundred seventy-eight rounds are complete")), "README count drift")
