...
def main() -> int:
    ...
    need(len(sources) == 15 and len(connectors) == 4, "source/connector coverage drift")
    ...
    need(all("CF9.UL.02.02" in r["power_pair_test_coupon_candidate"] and r["candidate_conductor_mm2"] == "0.250" for r in axes), "wire candidate drift")
    need(all("0.33 mm2" in r["jst_eh_published_conductor_range"] and r["wire_contact_geometric_compatibility"].startswith("CANDIDATE PASS") for r in axes), "wire/contact geometric evidence missing")
    need(all("AWG22 ONLY" in r["connector_current_evidence"] and r["current_capacity_disposition"].startswith("OPEN") for r in axes), "ampacity limitation overclaimed")
    ...
    for r in axes:
        ...
        need("NOT HOT/BUNDLED" in r["calculation_boundary"], f"planning boundary missing: {r['axis_id']}")
    ...
    need(status["cf9_jst_cross_section_geometry_compatible"] is True, "compatible-size candidate not recorded")
    ...
    need("The 25 actuator feeds now have a compatible-size flex-cable candidate" in page and "Do not crimp or connect" in page, "guide purpose/warning drift")
    ...
    print("PASS: HR-30 actuator cable kit binds a compatible-size CF9 flex candidate and verified 20 C planning calculations to all 25 feeds; ampacity/crimp/protection/authority remain open")
    return 0
