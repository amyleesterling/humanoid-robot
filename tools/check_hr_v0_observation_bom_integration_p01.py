...
def main() -> int:
    ...
    if bill.get("system_group_count") != 108 or bill.get("configuration_reconciliation") not in {gen.CID, "HR-V0-CONFIG-REC-P0.24", "HR-V0-CONFIG-REC-P0.25"} or bill.get("observation_bom_integration") != gen.ID or gen.ID not in bill.get("supporting_identifiers", []):
        errors.append("release BOM metadata is stale")
    if electrical.get("configuration_reconciliation") not in {gen.CID, "HR-V0-CONFIG-REC-P0.24", "HR-V0-CONFIG-REC-P0.25"} or electrical.get("observation_bom_integration") != gen.ID:
