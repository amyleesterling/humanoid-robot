...
def main() -> int:
    ...
    require(10.95 < identified < 10.98, "lightweight whole-body/onboard-envelope identified subtotal outside controlled P0.1 band")
    ...
    require(9.89 < float(summary["active_tether_dynamics_planning_mass_kg"]) < 9.92, "active tether planning mass is not inside the controlled sub-10 kg P0.1 band")
    require(0.08 < float(summary["active_tether_margin_to_program_maximum_kg"]) < 0.11, "active tether positive hard-limit margin outside controlled P0.1 band")
    aux = next(row for row in items if row["item_id"] == "EQ-P01-AUX-CONVERTER")
    require(aux["candidate_material_or_model"].startswith("120 x 58 mm three-rail carrier candidate with 3x RECOM REC30E-2405SZ") and abs(float(aux["planning_candidate_mass_kg"]) - 0.160) < 1e-9, "three-rail auxiliary module mass identity drift")
