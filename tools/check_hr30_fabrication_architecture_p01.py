...
def main() -> int:
    ...
    require(all(float(row["nominal_wall_mm"]) >= 1.2 and "SELECTION REQUIRED" in row["release_state"] for row in panels), "panel wall/release boundary missing")
    require(all(abs(float(row["nominal_wall_mm"]) - 1.2) < 1e-9 for row in panels if row["module"] in {"H01", "T01", "P01"}), "central/head 1.2 mm shell architecture drift")
