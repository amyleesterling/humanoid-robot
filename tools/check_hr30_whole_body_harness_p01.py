...
def main() -> int:
    ...
    need(len(terminal_binding) == len(connector_schedule) == 681, "logical terminal completeness spine missing")
    aux_refs = {r["reference"] for r in terminal_binding if r["sheet"] == "15_pelvis_aux_imu.kicad_sch"}
    need({"AUXCOM1", "AUXHMI1", "AUXCTL1", "AUXSTAR1"} <= aux_refs and "AUXD1" not in aux_refs, "three-rail auxiliary logical-terminal spine missing")
    ...
    need((status["axis_drop_count"], status["bus_branch_count"], status["corridor_count"], status["connector_boundary_count"], status["harness_assembly_count"], status["installed_equipment_binding_count"], status["service_loop_obligation_count"], status["logical_terminal_binding_count"]) == (25,8,12,33,14,64,25,681), "status counts drift")
