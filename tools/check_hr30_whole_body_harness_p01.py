...
def main() -> int:
    ...
    need(len(equipment) == len(installed) == 58 and {r["item_id"] for r in equipment} == {r["item_id"] for r in installed}, "installed equipment harness completeness missing")
    ...
    need(len(sources) == 11 and {"STM32H743ZIT6","ISOW1432DFMR","SN74LVC1T45DCKR","JST-GH","U2D2","U2D2-PHB","JST-EH"} <= {r["source_id"] for r in sources}, "primary source register incomplete")
    status = json.loads((OUT / "harness-status.json").read_text(encoding="utf-8"))
    need((status["axis_drop_count"], status["bus_branch_count"], status["corridor_count"], status["connector_boundary_count"], status["harness_assembly_count"], status["installed_equipment_binding_count"], status["service_loop_obligation_count"], status["logical_terminal_binding_count"]) == (25,8,12,33,14,58,25,667), "status counts drift")
