...
def main() -> int:
    ...
    need(len(terminal_binding) == len(connector_schedule) == 390, "logical terminal completeness spine missing")
    ...
    need((status["axis_drop_count"], status["bus_branch_count"], status["corridor_count"], status["connector_boundary_count"], status["harness_assembly_count"], status["installed_equipment_binding_count"], status["service_loop_obligation_count"], status["logical_terminal_binding_count"]) == (25,8,12,33,14,54,25,390), "status counts drift")
