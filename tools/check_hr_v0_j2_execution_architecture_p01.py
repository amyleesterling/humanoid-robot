...
def main() -> int:
    ...
    need((ROOT / "docs/handoff-current.md").read_text(encoding="utf-8").startswith(("R283 J2 execution architecture:", "R284 C07 curved-mesh development:", "R285 targeted C07 remesh:")), "handoff")
    need((ROOT / "docs/review-ledger.md").read_text(encoding="utf-8").count("| R283 |") == 1, "review ledger")
    need(any(text in (ROOT / "README.md").read_text(encoding="utf-8") for text in ("Two hundred eighty-three rounds are complete", "Two hundred eighty-four rounds are complete", "Two hundred eighty-five rounds are complete")), "README count")
