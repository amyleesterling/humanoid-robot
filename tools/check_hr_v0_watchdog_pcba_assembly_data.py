...
def main() -> int:
    ...
    if status.get("board_sha256") == hashlib.sha256(BOARD.read_bytes()).hexdigest():
        failures.append("historical R133 P0.7 hash unexpectedly equals current metadata-only P0.9 source")
    ...
    print("Historical PCB-P0.7: 42 populated refs, 16 exact-MPN BOM lines, 38 SMD / 4 THT, 4 NPTH; current PCB-P0.9 successor is controlled by R139")
