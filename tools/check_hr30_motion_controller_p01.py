...
def main() -> int:
    ...
    require(found is not None and int(found.group(1)) > 0, "blocked PCB DRC state is not explicit")
    categories: dict[str, int] = {}
    for value in re.findall(r"^\[([^]]+)\]", drc, re.MULTILINE): categories[value] = categories.get(value, 0) + 1
    require(categories.get("unconnected_items", 0) > 0 and categories.get("shorting_items", 0) > 0, "DRC blocker categories unexpectedly absent")
    status = json.loads((PKG / "controller-status.json").read_text(encoding="utf-8"))
    require(status["drc_violations"] == int(found.group(1)) and status["unconnected_item_count"] == categories["unconnected_items"], "status/report DRC mismatch")
    ...
    require(not status["routing_complete"] and not status["fabrication_release"] and not status["functional_safety_credit"], "blocked layout or safety boundary overclaimed")
    ...
    require(WARNING in guide and f">{status['drc_violations']}<" in guide and "font:17px/1.55" in guide and "not fabrication-ready" in guide, "web guide hides blocker or violates legibility")
    ...
    require(root_status["motion_controller_drc_violations"] == status["drc_violations"] and root_status["motion_controller_layout_blocked"], "root package hides controller layout blocker")
    ...
    print(f"PASS: HR-30 controller has six native sheets, ERC 0/0, corrected carrier/UART pins and a visible routed PCB candidate; {status['drc_violations']} DRC violations and {status['unconnected_item_count']} unconnected items remain explicit blockers; no fabrication, safety or work authority")
    return 0
