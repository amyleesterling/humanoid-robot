...
def source_bindings() -> list[dict[str, object]]:
    paths = [
        ...
        ("PCA-B06", "eight bus-source feed boundary", WHOLE / "electrical/tether-power-core-p0.1/eight-bus-feed-register.csv"),
        ("PCA-B07", "eight-board walking-power implementation status", WHOLE / "electrical/walking-power-successor-p0.1/walking-power-status.json"),
        ("PCA-B08", "walking-power unresolved holds", WHOLE / "electrical/walking-power-successor-p0.1/open-holds.csv"),
...
def power_states() -> list[dict[str, object]]:
    rows = [
        ...
        ("FER-E5", "ONE-BUS-BOARD BRANCH LOAD TEST", "RSP path through dual contactors", "one selected feed into approved electronic load", "one one-bus board only", "no actuators attached; all other feeds absent", "NO"),
        ("FER-E6", "WHOLE DISTRIBUTION NO LOAD", "RSP path through dual contactors", "eight feeds with individually selected protection", "eight one-bus boards; actuators absent", "source current limited below separately approved inspection limit", "NO"),
        ("FER-E7", "RESTRAINED WHOLE-ROBOT RAIL TEST", "RSP path through dual contactors", "eight bus-source feeds", "25 paired eFuse branches", "all prior gates complete; torque disabled; zero motion request", "NO"),
    ]
...
def protection_layers() -> list[dict[str, object]]:
    rows = [
        ...
        ("PCA-L04", "eight robot bus-source branches", "FB1-FB8 Littelfuse 04980923ZXT holders", "FER-E5 through FER-E7", "FUSE VALUES SELECTION REQUIRED", "RMS demand, fault current, inrush, regeneration, conductor/contact limits and coordination"),
        ("PCA-L05", "25 actuator branches", "paired opposing TPS259482LYWPR candidates", "FER-E7", "THRESHOLDS SELECTION REQUIRED", "threshold tolerance, hot PCB/connector tests, short-circuit behavior and feed-level brake/dump architecture"),
...
def bus_and_feed_rows() -> tuple[list[dict[str, object]], list[dict[str, object]], dict[str, float]]:
    ...
    feeds = read_csv(WHOLE / "electrical/tether-power-core-p0.1/eight-bus-feed-register.csv")
    feed_map = {
        "FB1": ["RS-LLEG"], "FB2": ["RS-RLEG"], "FB3": ["RS-LARM"], "FB4": ["RS-RARM"],
        "FB5": ["RS-WAIST"], "FB6": ["TTL-LDIST"], "FB7": ["TTL-RDIST"], "FB8": ["TTL-HEAD"],
    }
    ...
    for feed in feeds:
        ...
        feed_rows.append(common({
            "branch_id": feed["branch_id"], "feed_target": feed["feed_target"], "included_buses": "; ".join(members),
            "candidate_cap_sum_a": f"{cap:.6f}", "published_stall_endpoint_sum_a": f"{stall:.3f}",
            "terminal_nominal_a": "SELECTION REQUIRED", "holder_order_code": feed["holder_order_code"],
...
def closure_inputs() -> list[dict[str, object]]:
    data = [
        ...
        ("PCA-I06", "controller, walking-power board and actuator inrush by state", "A versus ms", "oscilloscope/current-probe capture", "NUISANCE TRIP/COORDINATION"),
        ("PCA-I07", "normal RMS and peak current for each axis and eight feeds", "A; duty cycle", "restrained staged motion measurements", "WIRE/FUSE THERMAL"),
        ...
        ("PCA-I09", "walking-power PCB copper, paired eFuse and connector temperature rise", "degC", "instrumented maximum-duty board test", "BOARD CAPACITY"),
        ...
        ("PCA-I14", "single-fault and backfeed behavior across all eight feeds and 25 branches", "fault-injection record", "guarded staged fault tests", "COORDINATION/SEPARATION"),
    ]
...
def page(status: dict[str, object], feed_rows: list[dict[str, object]], state_rows: list[dict[str, object]], hold_rows: list[dict[str, object]]) -> str:
    """Render current copy while retaining the proven responsive layout template."""
    return (
        _legacy_page_template(status, feed_rows, state_rows, hold_rows)
        .replace("five protected feeds, five PDU boards", "eight protected feeds, eight one-bus boards")
        .replace("Five whole-robot feeds", "Eight electrically separate whole-robot feeds")
        .replace("complete five-feed register", "complete eight-feed register")
        .replace("pdu-feed-envelope.csv", "bus-source-feed-envelope.csv")
    )
...
def integrate_root(status: dict[str, object]) -> None:
    ...
    root_status.pop("five_feed_fuses_selected", None)
    root_status.pop("pdu_feed_count", None)
    root_status.update({
        "protection_conductor_architecture_present": True,
        "protection_conductor_power_state_count": status["power_state_count"],
        "protection_conductor_feed_count": status["bus_feed_count"],
        ...
        "eight_feed_fuses_selected": False,
...
def main() -> int:
    ...
    status = {
        ...
        "protection_layer_count": 7, "bus_count": len(bus_rows), "bus_feed_count": len(feed_rows),
        ...
        "main_fuse_selected": False, "eight_feed_fuses_selected": False, "hot_conductor_ampacity_released": False,
    ...
    write_csv(OUT / "bus-source-feed-envelope.csv", feed_rows)
    ...
    (OUT / "protection-hierarchy.svg").write_text(diagram(), encoding="utf-8", newline="\n")
    (OUT / "protection-conductor-status.json").write_text(json.dumps(status, indent=2) + "\n", encoding="utf-8")
    (OUT / "index.html").write_text(page(status, feed_rows, state_rows, hold_rows), encoding="utf-8", newline="\n")
    (OUT / "README.md").write_text(
        f"# HR-30 protection and conductor architecture P0.1\n\n**{WARNING}**\n\nThis package separates the 5 V logic-only state from the 12 V actuator path and binds the latter through one source, dual interruption, eight electrically separate bus-source feeds and 25 paired-eFuse branches. It deliberately releases no fuse value or conductor ampacity. Open [index.html](index.html) for the interactive guide.\n",
