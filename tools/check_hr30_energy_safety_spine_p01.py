...
def main():
    ...
    req(any(r['device_id']=='DEV-04' and 'LC1D40ABD' in r['candidate'] and '21-22 NC auxiliary is mirror certified' in r['engineering_boundary'] for r in dev),"mirror-contact contactor correction missing")
    ...
    req(any(r['function_id']=='SF-03' and '21-22 mirror-certified NC' in r['input_architecture'] for r in safety),"EDM mirror-contact binding missing")
    ...
    req(status['tether_first_configuration_defined'] and status['direct_4s_lipo_architecture_rejected'] and status['native_kicad_topology_correction_complete'] and status['pin_level_kicad_correction_required'] is False and status['physical_energy_safety_terminal_release_required'] and status['individual_actuator_power_feed_count']==25,"status architecture drift")
    ...
    wbstatus=json.loads((WB/'package-status.json').read_text(encoding='utf-8')); req(wbstatus.get('energy_safety_spine_present') is True and wbstatus.get('tether_first_equipment_configuration') is True and wbstatus.get('energy_safety_native_kicad_topology_corrected') is True and wbstatus.get('energy_safety_native_kicad_correction_required') is False and wbstatus.get('energy_safety_physical_terminal_release_required') is True and wbstatus.get('energy_safety_individual_actuator_power_feed_count')==25 and wbstatus.get('energization_authority') is False,"whole-body integration status missing/overclaimed")
