...
def main() -> None:
    ...
    for product in release["current_products"]:
        if product.get("domain") in {"electrical","mechanical","bill_of_materials","commissioning","assembly"}:
            need(product.get("configuration_reconciliation") in {gen.CID,"HR-V0-CONFIG-REC-P0.32","HR-V0-CONFIG-REC-P0.33","HR-V0-CONFIG-REC-P0.34","HR-V0-CONFIG-REC-P0.35"},f"release config {product.get('domain')}")
