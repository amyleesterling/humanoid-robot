...
def main() -> int:
    ...
    assert status["installed_item_count"] == len(rows) >= 50
    ...
    assert sum(item.startswith("EQ-HN01_") for item in ids) == 11
    ...
    for name in ("installed-equipment-register.csv", "installed-equipment-source-register.csv",
