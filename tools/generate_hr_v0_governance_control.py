...
WEB = ROOT / "release" / "hr-v0" / "governance-p0.2"
IDENTIFIER = "HR-V0-GOV-P0.2"
DATE = "2026-08-09"
...
def main() -> int:
    ...
    for row in requirements:
        ...
        atomicity_rows.append({
            ...
            "child_requirement_register": "requirements/atomic-p0.1/atomic-requirements.csv" if compound else "NOT APPLICABLE CANDIDATE",
    ...
    summary = {
        ...
        "atomic_child_candidate_count": 396,
    ...
    page = f'''<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>HR-V0 governance control P0.2</title><style>
...
</style></head><body><div class="warning">{WARNING}</div><header><p>{IDENTIFIER} · R142 · {DATE}</p><h1>Accountability coverage with atomic child trace.</h1><p>This review surface covers every current requirement, risk and release gate and links the 66 compound parents to 396 draft child candidates. Roles are candidates; named people, signatures, executed evidence and row-level history remain open.</p></header><main><section class="cards"><div class="card"><div class="number">{len(requirement_rows)}</div>requirements</div><div class="card"><div class="number">{len(risk_rows)}</div>risks</div><div class="card"><div class="number">{len(gate_rows)}</div>gates</div><div class="card"><div class="number">396</div>draft atomic children</div><div class="card"><div class="number">0</div>approved records</div></section><section><h2>Controlled register</h2><div class="filters"><label>Search<input id="search" type="search" placeholder="ID, role, evidence or state"></label><label>Record type<select id="kind"><option value="">All</option><option>requirement</option><option>risk</option><option>gate</option></select></label></div><p id="count" aria-live="polite"></p><div class="table-wrap"><table><thead><tr><th>Record</th><th>Domain/stage</th><th>Accountability</th><th>Evidence</th><th>Approval</th><th>State</th></tr></thead><tbody id="rows"></tbody></table></div></section><section><h2>Fail-closed interpretation</h2><p>A role label is not a person, a file path is not executed evidence, and a clean checker is not approval. The 396 child IDs are draft candidates, not passed obligations. Every record remains preliminary until named accountable people and qualified approvers review configuration-bound evidence and sign a controlled decision.</p><p><a href="../../../requirements/governance-p0.2/requirement-control-register.csv">Requirements</a> · <a href="../../../requirements/governance-p0.2/risk-control-register.csv">Risks</a> · <a href="../../../requirements/governance-p0.2/gate-control-register.csv">Gates</a> · <a href="../../../requirements/governance-p0.2/requirement-atomicity-review.csv">Atomicity review</a> · <a href="../atomic-requirements-p0.1/index.html">Atomic children</a> · <a href="../../../requirements/governance-p0.2/governance-summary.json">Summary</a></p></section></main><footer>{WARNING}. GOV-001 and Sol B-018/N-004 remain open.</footer><script>
