...
def main()->int:
    ...
    need((ROOT/"docs/handoff-current.md").read_text(encoding="utf-8").startswith(("R280 J2 refinement execution feasibility:","R281 J2 numerical backend:","R282 J2 refinement erratum:","R283 J2 execution architecture:","R284 C07 curved-mesh development:")),"handoff")
    need((ROOT/"docs/review-ledger.md").read_text(encoding="utf-8").count("| R280 |")==1,"ledger")
    need(any(text in (ROOT/"README.md").read_text(encoding="utf-8") for text in ("Two hundred eighty rounds are complete","Two hundred eighty-one rounds are complete","Two hundred eighty-two rounds are complete","Two hundred eighty-three rounds are complete","Two hundred eighty-four rounds are complete")),"README")
