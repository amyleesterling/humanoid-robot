...
def main() -> int:
    ...
    need(status["dynamics_source"] == "hr30_tether.urdf" and abs(float(status["dynamics_mass_kg"]) - expected_mass) < 1e-6, "walking sequence is not bound to the active tether-first model")
    ...
    need(all(abs(float(row["mass_kg"]) - expected_mass) < 1e-6 for row in samples), "sample mass drift from tether-first dynamics")
