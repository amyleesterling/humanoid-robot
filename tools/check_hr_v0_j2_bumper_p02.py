...
def main() -> int:
    ...
    need((ROOT / "docs/handoff-current.md").read_text(encoding="utf-8").startswith("R276 exact-contact J2 pad correction:"), "handoff drift")
    need("| R276 |" in (ROOT / "docs/review-ledger.md").read_text(encoding="utf-8"), "ledger drift")
    need("Two hundred seventy-six rounds are complete" in (ROOT / "README.md").read_text(encoding="utf-8"), "README count drift")
