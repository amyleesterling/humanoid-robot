...
def main() -> int:
    ...
    need((ROOT / "docs/handoff-current.md").read_text(encoding="utf-8").startswith(("R276 exact-contact J2 pad correction:", "R277 J2 pad-pocket correction:")), "handoff drift")
    need("| R276 |" in (ROOT / "docs/review-ledger.md").read_text(encoding="utf-8"), "ledger drift")
    need(any(text in (ROOT / "README.md").read_text(encoding="utf-8") for text in ("Two hundred seventy-six rounds are complete", "Two hundred seventy-seven rounds are complete")), "README count drift")
