"""Validate HR-V0-DXL-STAR-MFG-P0.2 without granting work authority."""
...
def main() -> int:
    ...
    need(status.get("identifier") == "HR-V0-DXL-STAR-MFG-P0.2", "wrong identifier")
    need(status.get("round") == "R164", "wrong round")
    need(status.get("board") == "DXL-STAR-P0.2-CARRIER-CANDIDATE", "wrong board identity")
    ...
    need("HR-V0-DXL-STAR-MFG-P0.2" in electrical_product.get("supporting_identifiers", []), "current P0.2 CAM review is missing from electrical support")
    need("HR-V0-DXL-STAR-MFG-P0.2" in bom_product.get("supporting_identifiers", []), "current P0.2 CAM review is missing from BOM support")
    ...
    need(
        ...
        and "P0.1 CAM" in bom051.get("selection_basis", "")
        and "connector/current" in bom051.get("selection_basis", ""),
        "BOM-051 does not retain current P0.2 CAM hold and historical P0.1 boundary",
    )
    ...
    need(
        ...
        and "release/hr-v0/dxl-star-manufacturing-p0.2/" in gates.get("EG-004", {}).get("evidence_location", "")
        and "tools/check_hr_v0_dxl_star_manufacturing_p02.py" in gates.get("EG-004", {}).get("evidence_location", ""),
    ...
    source_map = {
        "electrical/kicad/hr-v0-dxl-star-p0.2-carrier-candidate/hr-v0-dxl-star-p0.2-carrier-candidate.kicad_pcb": SOURCE / "hr-v0-dxl-star-p0.2-carrier-candidate.kicad_pcb",
        "electrical/kicad/hr-v0-dxl-star-p0.2-carrier-candidate/hr-v0-dxl-star-p0.2-carrier-candidate.kicad_pro": SOURCE / "hr-v0-dxl-star-p0.2-carrier-candidate.kicad_pro",
        "electrical/kicad/hr-v0-dxl-star-p0.2-carrier-candidate/bom.csv": SOURCE / "bom.csv",
        "electrical/kicad/hr-v0-dxl-star-p0.2-carrier-candidate/connector-schedule.csv": SOURCE / "connector-schedule.csv",
    }
    ...
    need('gr_text "DXL-STAR-P0.2-CARRIER-CANDIDATE - U2D2 VDD OMITTED"' in board_text, "native revision/identity marking changed")
    ...
    for token in ("font:clamp(16px", "code{font-size:14px", "DXL-STAR-P0.2-CARRIER-CANDIDATE CAM exists for review", "quarantined internal evidence", "NOT MACHINE XYRS", "Eighteen holds remain open", "supplier releases or work authorizations"):
    ...
    if failures:
        print("HR-V0-DXL-STAR-MFG-P0.2 check FAILED")
        for failure in failures:
            print("-", failure)
        return 1
    print("HR-V0-DXL-STAR-MFG-P0.2 PASS")
    print("  DXL-STAR-P0.2-CARRIER-CANDIDATE source bound; native DRC 0; 10 Gerber/job + 5 drill/map/report files")
