...
def main() -> int:
    ...
    require(any(row["item_id"] == "HR30-BOM-002" and int(row["quantity"]) == 1 and row["function"] == "waist-yaw actuator" for row in bom), "XM540 waist allocation drift")
    require(any(row["item_id"] == "HR30-BOM-003" and int(row["quantity"]) == 10 and row["function"] == "shoulder/elbow/ankle actuator" for row in bom), "XM430 whole-body allocation drift")
    ...
    require(status.get("reduced_leg_drivetrain_package_present") and status.get("reduced_leg_drivetrain_module_count") == 4 and status.get("reduced_leg_drivetrain_axis_count") == 10 and status.get("reduced_leg_drivetrain_candidate_products_defined"), "whole-body reduced-leg drivetrain package missing")
    ...
    require(status["whole_body_pose_count"] == 10, "bilateral articulated pose set incomplete")
    ...
    require(status["installed_equipment_item_count"] == 64 and status["tether_first_equipment_configuration"] and status["tether_development_interface_retained"] and status["onboard_energy_candidate_geometry_present"] and not status["onboard_energy_installed"], "installed-equipment configuration boundary drift")
    ...
    print(f"PASS: HR-30 whole-body P0.1 has 25-DOF URDF/MJCF, ten bilateral articulated S2-S5 pose candidates including grounded touchdown configurations, the existing screened pose set has zero nominal common-volume interference, 64 located equipment items including eight isolated one-bus walking-power boards, three dedicated 9 V regulators and two four-channel interface-carrier reservations, 14 harness assemblies with 25 individually routed actuator-power pairs, separate head power/data routes, tether-first development power, and {reconciled_mass:.3f} kg planning mass plus budgets, BOM, hands, walking, agent and build artifacts; tolerance/physical validation and all work authority remain false")
    return 0
