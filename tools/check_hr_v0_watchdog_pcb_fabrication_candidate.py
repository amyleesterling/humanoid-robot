...
def main() -> int:
    ...
    need(sha256(controlled_board) != sha256(SOURCE / controlled_board.name),
         "historical PCB-P0.5 unexpectedly equals current PCB-P1.0 source", failures)
    ...
    need('rev "PCB-P1.0 / Electrical V3-P1.15"' in (SOURCE / controlled_board.name).read_text(encoding="utf-8-sig"),
         "live source is not the controlled direct-bound PCB-P1.0 successor", failures)
    ...
    print("  superseded by direct-bound PCB-P1.0 for current review; do not upload or order PCB-P0.5")
