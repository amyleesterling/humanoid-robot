...
def rows() -> dict[str, list[dict[str, object]]]:
    ...
    gates = [
        ...
        ("EG-004", "electrical", "partial", "supplier-normalized watchdog data, provider/process acceptance, physical terminals, ratings, wiring, tests and qualified review"),
...
def html_r166() -> str:
    return (
        html()
        .replace(
            "R163 reconciles the carrier-aware chain. R164 adds current P0.2 CAM review evidence. R165 proves exact P1.15 digital parity for the unchanged watchdog/E2 subset and issues a fail-closed P0.4 E2 slice.",
            "R163 reconciles the carrier-aware chain. R164 adds actuator-star P0.2 CAM review evidence. R165 proves exact P1.15 digital parity for the unchanged watchdog/E2 subset and issues a fail-closed P0.4 E2 slice. R166 binds watchdog CAM P0.2 to the P1.15 source and parity evidence.",
        )
        .replace(
            "</tr></table><h2>Gate effect</h2>",
            "</tr><tr><td>Watchdog CAM P0.1 was tied to historical P1.14.</td><td>R166 regenerates P0.2 and hash-binds P1.15 plus R165 parity evidence.</td><td>Supplier-normalized XYRS, provider/process acceptance, first article and work authority remain open.</td></tr></table><h2>Gate effect</h2>",
        )
    )
...
def main() -> None:
    ...
    (OUT / "index.html").write_text(html_r166(), encoding="utf-8")
