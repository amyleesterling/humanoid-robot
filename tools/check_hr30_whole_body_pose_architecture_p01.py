...
def main() -> int:
    ...
    require(all(abs(float(row["total_mass_kg"]) - expected_mass) < 5e-6 for row in metrics), "pose metrics are not bound to the active tether-first dynamics mass")
    ...
    require(status["pose_dynamics_source"] == "hr30_tether.urdf" and abs(float(status["pose_dynamics_mass_kg"]) - expected_mass) < 5e-6, "pose status does not identify the active tether-first dynamics source")
