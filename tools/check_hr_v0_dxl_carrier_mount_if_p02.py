...
def main() -> None:
    ...
    for product in release["current_products"]:
        if product.get("domain") in {"electrical","bill_of_materials","assembly"}:
            need(product.get("configuration_reconciliation") in {gen.CID,"HR-V0-CONFIG-REC-P0.29","HR-V0-CONFIG-REC-P0.30","HR-V0-CONFIG-REC-P0.31","HR-V0-CONFIG-REC-P0.32"} and product.get("dxl_carrier_mount")==gen.ID,"release metadata stale")
