...
def main() -> int:
    ...
    require(len(summaries) == 8 and len(pairs) == 2320, "bilateral pose collision coverage incomplete")
    ...
    require("2,320 checked" in hold["unresolved_item"] and "physical correlation remain unverified" in hold["unresolved_item"], "H08 collision progress/boundary not synchronized")
    ...
    print(f"PASS: eight bilateral HR-30 articulated poses screen 2,320 nonexcluded link pairs with zero nominal common-volume interference and {minimum:.2f} mm minimum clearance; tolerance, covers, cables, tracking error and physical correlation remain open with no motion/safety authority")
    return 0
