...
def source_bindings() -> list[dict[str, object]]:
    items = [
        ...
        ("BR-B11", "logic-only power-kit status", WHOLE / "electrical/logic-power-kit-p0.1/logic-power-status.json"),
        ("BR-B12", "logic-only power contact map", WHOLE / "electrical/logic-power-kit-p0.1/connector-contact-map.csv"),
    ]
...
def bom_rows() -> list[dict[str, object]]:
    data = [
        ...
        ("BR-P09", "logic-only target supply", "SIGLENT", "SPD3303X", 1, "exact isolated programmable supply candidate; receipt, calibration, limits and reference approval open", "EXACT CANDIDATE - PHYSICAL/SETTING REVIEW OPEN"),
    ]
...
def hold_rows() -> list[dict[str, object]]:
    data = [
        ...
        ("BR-H03", "logic-only power source", "SPD3303X and exact two-conductor cable candidates exist; receive/calibrate supply, build/inspect cable and freeze voltage/current/OCP/reference against the received controller"),
...
def main() -> int:
    ...
    status = {
        ...
        "logic_supply_exact_candidate_selected": True,
        "logic_supply_cable_candidate_selected": True,
