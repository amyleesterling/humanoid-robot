...
def main() -> None:
    ...
    require(len(overlay) == 19, "overlay manifest must contain nineteen proposed files")
    require(
        len(holds) == 18
    ...
    require(len({row["target"] for row in overlay}) == 19, "overlay target path is duplicated")
    ...
    require("24" in doc and "seventeen" in doc.lower() and "one partial" in doc.lower() and "21" in doc, "documented hold/evidence counts changed")
    ...
    print("HR-V0 host deployment P0.1 check passed: 19-file disabled overlay, 24 current preflight holds, 17 open plus 1 partial closure hold, 21 unexecuted evidence rows, 8 host tests")
